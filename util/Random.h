...
namespace RandomImpl {
    ...
    inline consteval uint32_t CxPRNG(int16_t n = 1, std::array<char, 9> seed = std::array<char, 9>{__TIME__}) {
...
inline consteval int32_t RandIntCx(const int32_t min, const int32_t max, const int16_t n = 2) noexcept {
    if (min >= max)
        return min;
...
inline consteval bool RandBoolCx(int16_t n = 3) noexcept
