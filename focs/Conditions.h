...
struct FO_COMMON_API focs::Number final : public Condition {
    Number(std::unique_ptr<ValueRef<int>>&& low,
           std::unique_ptr<ValueRef<int>>&& high,
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_low;
    std::unique_ptr<ValueRef<int>> m_high;
    std::unique_ptr<Condition> m_condition;
};

/** Matches all objects if the current game turn is >= \a low and < \a high. */
struct FO_COMMON_API focs::Turn final : public Condition {
    explicit Turn(std::unique_ptr<ValueRef<int>>&& low,
                  std::unique_ptr<ValueRef<int>>&& high = nullptr);
...
struct FO_COMMON_API focs::SortedNumberOf final : public Condition {
    /** Sorts randomly, without considering a sort key. */
    SortedNumberOf(std::unique_ptr<ValueRef<int>>&& number,
                   std::unique_ptr<Condition>&& condition);

    /** Sorts according to the specified method, based on the key values
      * evaluated for each object. */
    SortedNumberOf(std::unique_ptr<ValueRef<int>>&& number,
                   std::unique_ptr<ValueRef<double>>&& sort_key_ref,
...
private:
    std::unique_ptr<ValueRef<int>> m_number;
    std::unique_ptr<ValueRef<double>> m_sort_key;
...
struct FO_COMMON_API focs::None final : public Condition {
    None();
    bool operator==(const Condition& rhs) const override;
    void Eval(const ScriptingContext& parent_context, ObjectSet& matches,
              ObjectSet& non_matches, SearchDomain search_domain = NON_MATCHES) const override;
...
struct FO_COMMON_API focs::EmpireAffiliation final : public Condition {
    EmpireAffiliation(std::unique_ptr<ValueRef<int>>&& empire_id, EmpireAffiliationType affiliation);
    explicit EmpireAffiliation(std::unique_ptr<ValueRef<int>>&& empire_id);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_empire_id;
...
struct FO_COMMON_API focs::RootCandidate final : public Condition {
    RootCandidate();
    bool operator==(const Condition& rhs) const override;
    void GetDefaultInitialCandidateObjects(const ScriptingContext& parent_context,
                                           ObjectSet& condition_non_targets) const override;
...
struct FO_COMMON_API focs::Target final : public Condition {
    Target();
    bool operator==(const Condition& rhs) const override;
    void GetDefaultInitialCandidateObjects(const ScriptingContext& parent_context,
                                           ObjectSet& condition_non_targets) const override;
...
struct FO_COMMON_API focs::Homeworld final : public Condition {
    Homeworld();
    explicit Homeworld(std::vector<std::unique_ptr<ValueRef<std::string>>>&& names);
...
private:
    ...
    std::vector<std::unique_ptr<ValueRef<std::string>>> m_names;
};

/** Matches planets that are an empire's capital. */
struct FO_COMMON_API focs::Capital final : public Condition {
    Capital();
    bool operator==(const Condition& rhs) const override;
    void GetDefaultInitialCandidateObjects(const ScriptingContext& parent_context,
                                           ObjectSet& condition_non_targets) const override;
...
struct FO_COMMON_API focs::Monster final : public Condition {
    Monster();
    bool operator==(const Condition& rhs) const override;
    void GetDefaultInitialCandidateObjects(const ScriptingContext& parent_context,
                                           ObjectSet& condition_non_targets) const override;
...
struct FO_COMMON_API focs::Armed final : public Condition {
    Armed();
    bool operator==(const Condition& rhs) const override;
    std::string Description(bool negated = false) const override;
    std::string Dump(unsigned short ntabs = 0) const override;
...
struct FO_COMMON_API focs::Type final : public Condition {
    explicit Type(std::unique_ptr<ValueRef<UniverseObjectType>>&& type);
...
private:
    ...
    std::unique_ptr<ValueRef<UniverseObjectType>> m_type;
};

/** Matches all Building objects that are one of the building types specified
  * in \a names. */
struct FO_COMMON_API focs::Building final : public Condition {
    explicit Building(std::vector<std::unique_ptr<ValueRef<std::string>>>&& names);
...
struct FO_COMMON_API focs::HasSpecial final : public Condition {
    HasSpecial();
    explicit HasSpecial(std::string name);
    explicit HasSpecial(std::unique_ptr<ValueRef<std::string>>&& name);
    HasSpecial(std::unique_ptr<ValueRef<std::string>>&& name,
               std::unique_ptr<ValueRef<int>>&& since_turn_low,
               std::unique_ptr<ValueRef<int>>&& since_turn_high = nullptr);
    HasSpecial(std::unique_ptr<ValueRef<std::string>>&& name,
               std::unique_ptr<ValueRef<double>>&& capacity_low,
               std::unique_ptr<ValueRef<double>>&& capacity_high = nullptr);
...
private:
    ...
    std::unique_ptr<ValueRef<std::string>>  m_name;
    std::unique_ptr<ValueRef<double>>       m_capacity_low;
    std::unique_ptr<ValueRef<double>>       m_capacity_high;
    std::unique_ptr<ValueRef<int>>          m_since_turn_low;
    std::unique_ptr<ValueRef<int>>          m_since_turn_high;
};

/** Matches all objects that have the tag \a tag. */
struct FO_COMMON_API focs::HasTag final : public Condition {
    HasTag();
    explicit HasTag(const std::string& name);
    explicit HasTag(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::CreatedOnTurn final : public Condition {
    CreatedOnTurn(std::unique_ptr<ValueRef<int>>&& low,
                  std::unique_ptr<ValueRef<int>>&& high);
...
struct FO_COMMON_API focs::Contains final : public Condition {
...
/** Matches all objects that are contained by an object that matches Condition
  * \a condition.  Container objects are Systems, Planets (which contain
  * Buildings), and Fleets (which contain Ships). */
struct FO_COMMON_API focs::ContainedBy final : public Condition {
...
struct FO_COMMON_API focs::InOrIsSystem final : public Condition {
    InOrIsSystem(std::unique_ptr<ValueRef<int>>&& system_id);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_system_id;
};
...
struct FO_COMMON_API focs::OnPlanet final : public Condition {
    OnPlanet(std::unique_ptr<ValueRef<int>>&& planet_id);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_planet_id;
};

/** Matches the object with the id \a object_id */
struct FO_COMMON_API focs::ObjectID final : public Condition {
    ObjectID(std::unique_ptr<ValueRef<int>>&& object_id);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_object_id;
};
...
/** Matches all Planet objects that have one of the PlanetTypes in \a types.
  ...
  * matched. */
struct FO_COMMON_API focs::PlanetType final : public Condition {
    PlanetType(std::vector<std::unique_ptr<ValueRef< ::PlanetType>>>&& types);
...
private:
    ...
    std::vector<std::unique_ptr<ValueRef<::PlanetType>>> m_types;
};

/** Matches all Planet objects that have one of the PlanetSizes in \a sizes.
  * Note that all Building objects which are on matching planets are also
  * matched. */
struct FO_COMMON_API focs::PlanetSize final : public Condition {
    PlanetSize(std::vector<std::unique_ptr<ValueRef< ::PlanetSize>>>&& sizes);
...
private:
    ...
    std::vector<std::unique_ptr<ValueRef<::PlanetSize>>> m_sizes;
};
...
struct FO_COMMON_API focs::PlanetEnvironment final : public Condition {
    PlanetEnvironment(std::vector<std::unique_ptr<ValueRef< ::PlanetEnvironment>>>&& environments,
                      std::unique_ptr<ValueRef<std::string>>&& species_name_ref = nullptr);
...
private:
    ...
    std::vector<std::unique_ptr<ValueRef<::PlanetEnvironment>>> m_environments;
    std::unique_ptr<ValueRef<std::string>> m_species_name;
};
...
/** Matches all planets or ships that have one of the species in \a species.
  ...
  * matched. */
struct FO_COMMON_API focs::Species final : public Condition {
    explicit Species(std::vector<std::unique_ptr<ValueRef<std::string>>>&& names);
    Species();
...
struct FO_COMMON_API focs::Enqueued final : public Condition {
    Enqueued(BuildType build_type,
             std::unique_ptr<ValueRef<std::string>>&& name,
             std::unique_ptr<ValueRef<int>>&& empire_id = nullptr,
             std::unique_ptr<ValueRef<int>>&& low = nullptr,
             std::unique_ptr<ValueRef<int>>&& high = nullptr);
    explicit Enqueued(std::unique_ptr<ValueRef<int>>&& design_id,
                      std::unique_ptr<ValueRef<int>>&& empire_id = nullptr,
                      std::unique_ptr<ValueRef<int>>&& low = nullptr,
                      std::unique_ptr<ValueRef<int>>&& high = nullptr);
    Enqueued();
...
private:
    ...
    std::unique_ptr<ValueRef<std::string>>  m_name;
    std::unique_ptr<ValueRef<int>>          m_design_id;
    std::unique_ptr<ValueRef<int>>          m_empire_id;
    std::unique_ptr<ValueRef<int>>          m_low;
    std::unique_ptr<ValueRef<int>>          m_high;
};
...
struct FO_COMMON_API focs::FocusType final : public Condition {
    FocusType(std::vector<std::unique_ptr<ValueRef<std::string>>>&& names);
...
struct FO_COMMON_API focs::StarType final : public Condition {
    StarType(std::vector<std::unique_ptr<ValueRef< ::StarType>>>&& types);
...
private:
    ...
    std::vector<std::unique_ptr<ValueRef<::StarType>>> m_types;
};

/** Matches all ships whose ShipDesign has the hull specified by \a name. */
struct FO_COMMON_API focs::DesignHasHull final : public Condition {
    explicit DesignHasHull(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::DesignHasPart final : public Condition {
    DesignHasPart(std::unique_ptr<ValueRef<std::string>>&& name,
                  std::unique_ptr<ValueRef<int>>&& low = nullptr,
                  std::unique_ptr<ValueRef<int>>&& high = nullptr);
...
struct FO_COMMON_API focs::DesignHasPartClass final : public Condition {
    DesignHasPartClass(ShipPartClass part_class,
                       std::unique_ptr<ValueRef<int>>&& low,
                       std::unique_ptr<ValueRef<int>>&& high);
...
/** Matches ships who ShipDesign is a predefined shipdesign with the name
  * \a name */
struct FO_COMMON_API focs::PredefinedShipDesign final : public Condition {
    explicit PredefinedShipDesign(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::NumberedShipDesign final : public Condition {
    explicit NumberedShipDesign(std::unique_ptr<ValueRef<int>>&& design_id);
...
struct FO_COMMON_API focs::ProducedByEmpire final : public Condition {
    explicit ProducedByEmpire(std::unique_ptr<ValueRef<int>>&& empire_id);
...
struct FO_COMMON_API focs::Chance final : public Condition {
    explicit Chance(std::unique_ptr<ValueRef<double>>&& chance);
...
private:
    ...
    std::unique_ptr<ValueRef<double>> m_chance;
};

/** Matches all objects that have a meter of type \a meter, and whose current
  * value is >= \a low and <= \a high. */
struct FO_COMMON_API focs::MeterValue final : public Condition {
    MeterValue(MeterType meter,
               std::unique_ptr<ValueRef<double>>&& low,
               std::unique_ptr<ValueRef<double>>&& high);
...
private:
    ...
    MeterType m_meter;
    std::unique_ptr<ValueRef<double>> m_low;
    std::unique_ptr<ValueRef<double>> m_high;
};

/** Matches ships that have a ship part meter of type \a meter for part \a part
  * whose current value is >= low and <= high. */
struct FO_COMMON_API focs::ShipPartMeterValue final : public Condition {
    ShipPartMeterValue(std::unique_ptr<ValueRef<std::string>>&& ship_part_name,
                       MeterType meter,
                       std::unique_ptr<ValueRef<double>>&& low,
                       std::unique_ptr<ValueRef<double>>&& high);
...
private:
    ...
    std::unique_ptr<ValueRef<std::string>> m_part_name;
    MeterType m_meter;
    std::unique_ptr<ValueRef<double>> m_low;
    std::unique_ptr<ValueRef<double>> m_high;
};
...
struct FO_COMMON_API focs::EmpireMeterValue final : public Condition {
    EmpireMeterValue(std::string meter,
                     std::unique_ptr<ValueRef<double>>&& low,
                     std::unique_ptr<ValueRef<double>>&& high);
    EmpireMeterValue(std::unique_ptr<ValueRef<int>>&& empire_id,
                     std::string meter,
                     std::unique_ptr<ValueRef<double>>&& low,
                     std::unique_ptr<ValueRef<double>>&& high);
...
struct FO_COMMON_API focs::EmpireStockpileValue final : public Condition {
    EmpireStockpileValue(ResourceType stockpile,
                         std::unique_ptr<ValueRef<double>>&& low,
                         std::unique_ptr<ValueRef<double>>&& high);
    EmpireStockpileValue(std::unique_ptr<ValueRef<int>>&& empire_id,
...
struct FO_COMMON_API focs::EmpireHasAdoptedPolicy final : public Condition {
    EmpireHasAdoptedPolicy(std::unique_ptr<ValueRef<int>>&& empire_id,
                           std::unique_ptr<ValueRef<std::string>>&& name);
    explicit EmpireHasAdoptedPolicy(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::OwnerHasTech final : public Condition {
    OwnerHasTech(std::unique_ptr<ValueRef<int>>&& empire_id,
                 std::unique_ptr<ValueRef<std::string>>&& name);
    explicit OwnerHasTech(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::OwnerHasBuildingTypeAvailable final : public Condition {
    OwnerHasBuildingTypeAvailable(std::unique_ptr<ValueRef<int>>&& empire_id,
                                  std::unique_ptr<ValueRef<std::string>>&& name);
    explicit OwnerHasBuildingTypeAvailable(const std::string& name);
    explicit OwnerHasBuildingTypeAvailable(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::OwnerHasShipDesignAvailable final : public Condition {
    OwnerHasShipDesignAvailable(std::unique_ptr<ValueRef<int>>&& empire_id,
                                std::unique_ptr<ValueRef<int>>&& design_id);
    explicit OwnerHasShipDesignAvailable(int design_id);
    explicit OwnerHasShipDesignAvailable(std::unique_ptr<ValueRef<int>>&& design_id);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_id;
    std::unique_ptr<ValueRef<int>> m_empire_id;
};

/** Matches all objects whose owner who has the ship part @a name available. */
struct FO_COMMON_API focs::OwnerHasShipPartAvailable final : public Condition {
    OwnerHasShipPartAvailable(std::unique_ptr<ValueRef<int>>&& empire_id,
                              std::unique_ptr<ValueRef<std::string>>&& name);
    explicit OwnerHasShipPartAvailable(const std::string& name);
    explicit OwnerHasShipPartAvailable(std::unique_ptr<ValueRef<std::string>>&& name);
...
struct FO_COMMON_API focs::VisibleToEmpire final : public Condition {
    explicit VisibleToEmpire(std::unique_ptr<ValueRef<int>>&& empire_id);
...
struct FO_COMMON_API focs::WithinDistance final : public Condition {
    WithinDistance(std::unique_ptr<ValueRef<double>>&& distance,
                   std::unique_ptr<Condition>&& condition);
...
private:
    ...
    std::unique_ptr<ValueRef<double>> m_distance;
    std::unique_ptr<Condition> m_condition;
};
...
/** Matches all objects that are within \a jumps starlane jumps of at least one
  ...
  * down considerably if overused.  It is best to use Conditions that yield
  * relatively few matches. */
struct FO_COMMON_API focs::WithinStarlaneJumps final : public Condition {
    WithinStarlaneJumps(std::unique_ptr<ValueRef<int>>&& jumps,
                        std::unique_ptr<Condition>&& condition);
...
private:
    ...
    std::unique_ptr<ValueRef<int>> m_jumps;
    std::unique_ptr<Condition> m_condition;
};
...
/** Matches objects that are in systems that could have starlanes added between
  ...
  * to an existing lane. */
struct FO_COMMON_API focs::CanAddStarlaneConnection : Condition {
...
/** Matches systems that have been explored by at least one Empire
  * in \a empire_ids. */
struct FO_COMMON_API focs::ExploredByEmpire final : public Condition {
    explicit ExploredByEmpire(std::unique_ptr<ValueRef<int>>&& empire_id);
...
struct FO_COMMON_API focs::Stationary final : public Condition {
    Stationary();
...
struct FO_COMMON_API focs::Aggressive final : public Condition {
    Aggressive();
...
struct FO_COMMON_API focs::FleetSupplyableByEmpire final : public Condition {
    explicit FleetSupplyableByEmpire(std::unique_ptr<ValueRef<int>>&& empire_id);
...
struct FO_COMMON_API focs::ResourceSupplyConnectedByEmpire final : public Condition {
    ResourceSupplyConnectedByEmpire(std::unique_ptr<ValueRef<int>>&& empire_id,
                                    std::unique_ptr<Condition>&& condition);
...
struct FO_COMMON_API focs::CanColonize final : public Condition {
    CanColonize();
...
struct FO_COMMON_API focs::CanProduceShips final : public Condition {
    CanProduceShips();
...
struct FO_COMMON_API focs::OrderedBombarded final : public Condition {
...
struct FO_COMMON_API focs::ValueTest final : public Condition {
    ValueTest(std::unique_ptr<ValueRef<double>>&& value_ref1,
              ComparisonType comp1,
              std::unique_ptr<ValueRef<double>>&& value_ref2,
              ComparisonType comp2 = INVALID_COMPARISON,
              std::unique_ptr<ValueRef<double>>&& value_ref3 = nullptr);

    ValueTest(std::unique_ptr<ValueRef<std::string>>&& value_ref1,
              ComparisonType comp1,
              std::unique_ptr<ValueRef<std::string>>&& value_ref2,
              ComparisonType comp2 = INVALID_COMPARISON,
              std::unique_ptr<ValueRef<std::string>>&& value_ref3 = nullptr);

    ValueTest(std::unique_ptr<ValueRef<int>>&& value_ref1,
              ComparisonType comp1,
              std::unique_ptr<ValueRef<int>>&& value_ref2,
              ComparisonType comp2 = INVALID_COMPARISON,
              std::unique_ptr<ValueRef<int>>&& value_ref3 = nullptr);
...
private:
    ...
    std::unique_ptr<ValueRef<double>> m_value_ref1;
    std::unique_ptr<ValueRef<double>> m_value_ref2;
    std::unique_ptr<ValueRef<double>> m_value_ref3;
    std::unique_ptr<ValueRef<std::string>> m_string_value_ref1;
    std::unique_ptr<ValueRef<std::string>> m_string_value_ref2;
    std::unique_ptr<ValueRef<std::string>> m_string_value_ref3;
    std::unique_ptr<ValueRef<int>> m_int_value_ref1;
    std::unique_ptr<ValueRef<int>> m_int_value_ref2;
    std::unique_ptr<ValueRef<int>> m_int_value_ref3;
...
/** Matches objects that match the location condition of the specified
  * content.  */
struct FO_COMMON_API focs::Location final : public Condition {
public:
    Location(ContentType content_type,
             std::unique_ptr<ValueRef<std::string>>&& name1,
             std::unique_ptr<ValueRef<std::string>>&& name2 = nullptr);
...
private:
    ...
    std::unique_ptr<ValueRef<std::string>> m_name1;
    std::unique_ptr<ValueRef<std::string>> m_name2;
    ContentType m_content_type;
};

/** Matches objects that match the combat targeting condition of the specified
  * content.  */
struct FO_COMMON_API focs::CombatTarget final : public Condition {
public:
...
struct FO_COMMON_API focs::And final : public Condition {
...
struct FO_COMMON_API focs::Or final : public Condition {
...
struct FO_COMMON_API focs::Not final : public Condition {
...
struct FO_COMMON_API focs::OrderedAlternativesOf final : public Condition {
...
struct FO_COMMON_API focs::Described final : public Condition {
