...
class FO_COMMON_API focs::NoOp final : public Effect {
public:
    NoOp();
...
/** Sets the meter of the given kind to \a value.  The max value of the meter
  ...
  * done. */
class FO_COMMON_API focs::SetMeter final : public Effect {
public:
    SetMeter(MeterType meter,
             std::unique_ptr<ValueRef<double>>&& value,
...
private:
    MeterType m_meter;
    std::unique_ptr<ValueRef<double>> m_value;
...
class FO_COMMON_API focs::SetShipPartMeter final : public Effect {
public:
    /** Affects the \a meter_type meter that belongs to part(s) named \a
        part_name. */
    SetShipPartMeter(MeterType meter_type,
                     std::unique_ptr<ValueRef<std::string>>&& part_name,
                     std::unique_ptr<ValueRef<double>>&& value);
    ...
    bool            IsMeterEffect() const override { return true; }
    void            SetTopLevelContent(const std::string& content_name) override;
    const           ValueRef<std::string>* GetPartName() const { return m_part_name.get(); }
    MeterType       GetMeterType() const { return m_meter; }
    unsigned int    GetCheckSum() const override;
...
/** Sets the indicated meter on the empire with the indicated id to the
  ...
  * does nothing. */
class FO_COMMON_API focs::SetEmpireMeter final : public Effect {
public:
    SetEmpireMeter(std::string& meter, std::unique_ptr<ValueRef<double>>&& value);

    SetEmpireMeter(std::unique_ptr<ValueRef<int>>&& empire_id, std::string& meter,
                   std::unique_ptr<ValueRef<double>>&& value);
...
class FO_COMMON_API focs::SetEmpireStockpile final : public Effect {
public:
    SetEmpireStockpile(ResourceType stockpile,
                       std::unique_ptr<ValueRef<double>>&& value);
    SetEmpireStockpile(std::unique_ptr<ValueRef<int>>&& empire_id,
...
class FO_COMMON_API focs::SetEmpireCapital final : public Effect {
public:
    explicit SetEmpireCapital();
    explicit SetEmpireCapital(std::unique_ptr<ValueRef<int>>&& empire_id);
...
class FO_COMMON_API focs::SetPlanetType final : public Effect {
public:
    explicit SetPlanetType(std::unique_ptr<ValueRef<::PlanetType>>&& type);
...
private:
    std::unique_ptr<ValueRef<::PlanetType>> m_type;
};
...
class FO_COMMON_API focs::SetPlanetSize final : public Effect {
public:
    explicit SetPlanetSize(std::unique_ptr<ValueRef<::PlanetSize>>&& size);
...
private:
    std::unique_ptr<ValueRef<::PlanetSize>> m_size;
};

/** Sets the species on the target to \a species_name.  This works on planets
  * and ships, but has no effect on other objects. */
class FO_COMMON_API focs::SetSpecies final : public Effect {
public:
    explicit SetSpecies(std::unique_ptr<ValueRef<std::string>>&& species);
...
class FO_COMMON_API focs::SetOwner final : public Effect {
public:
    explicit SetOwner(std::unique_ptr<ValueRef<int>>&& empire_id);
...
/** Sets the opinion of Species \a species for empire with id \a empire_id to
  * \a opinion */
class FO_COMMON_API focs::SetSpeciesEmpireOpinion final : public Effect {
public:
    SetSpeciesEmpireOpinion(std::unique_ptr<ValueRef<std::string>>&& species_name,
                            std::unique_ptr<ValueRef<int>>&& empire_id,
                            std::unique_ptr<ValueRef<double>>&& opinion);
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_species_name;
    std::unique_ptr<ValueRef<int>>            m_empire_id;
    std::unique_ptr<ValueRef<double>>         m_opinion;
};

/** Sets the opinion of Species \a opinionated_species for other species
  * \a rated_species to \a opinion */
class FO_COMMON_API focs::SetSpeciesSpeciesOpinion final : public Effect {
public:
    SetSpeciesSpeciesOpinion(std::unique_ptr<ValueRef<std::string>>&& opinionated_species_name,
                             std::unique_ptr<ValueRef<std::string>>&& rated_species_name,
                             std::unique_ptr<ValueRef<double>>&& opinion);
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_opinionated_species_name;
    std::unique_ptr<ValueRef<std::string>>    m_rated_species_name;
    std::unique_ptr<ValueRef<double>>         m_opinion;
};

/** Creates a new Planet with specified \a type and \a size at the system with
  * specified \a location_id */
class FO_COMMON_API focs::CreatePlanet final : public Effect {
public:
    CreatePlanet(std::unique_ptr<ValueRef<::PlanetType>>&& type,
                 std::unique_ptr<ValueRef<::PlanetSize>>&& size,
...
class FO_COMMON_API focs::CreateBuilding final : public Effect {
public:
    CreateBuilding(std::unique_ptr<ValueRef<std::string>>&& building_type_name,
                   std::unique_ptr<ValueRef<std::string>>&& name,
                   std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_building_type_name;
    std::unique_ptr<ValueRef<std::string>>    m_name;
    std::vector<std::unique_ptr<Effect>>                m_effects_to_apply_after;
};
...
class FO_COMMON_API focs::CreateShip final : public Effect {
public:
    CreateShip(std::unique_ptr<ValueRef<std::string>>&& predefined_ship_design_name,
               std::unique_ptr<ValueRef<int>>&& empire_id,
               std::unique_ptr<ValueRef<std::string>>&& species_name,
               std::unique_ptr<ValueRef<std::string>>&& ship_name,
               std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);

    CreateShip(std::unique_ptr<ValueRef<int>>&& ship_design_id,
               std::unique_ptr<ValueRef<int>>&& empire_id,
               std::unique_ptr<ValueRef<std::string>>&& species_name,
               std::unique_ptr<ValueRef<std::string>>&& ship_name,
               std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_design_name;
...
class FO_COMMON_API focs::CreateField final : public Effect {
public:
    CreateField(std::unique_ptr<ValueRef<std::string>>&& field_type_name,
                std::unique_ptr<ValueRef<double>>&& size,
                std::unique_ptr<ValueRef<std::string>>&& name,
                std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);

    CreateField(std::unique_ptr<ValueRef<std::string>>&& field_type_name,
                std::unique_ptr<ValueRef<double>>&& x,
                std::unique_ptr<ValueRef<double>>&& y,
                std::unique_ptr<ValueRef<double>>&& size,
                std::unique_ptr<ValueRef<std::string>>&& name,
                std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_field_type_name;
    std::unique_ptr<ValueRef<double>>         m_x;
    std::unique_ptr<ValueRef<double>>         m_y;
    std::unique_ptr<ValueRef<double>>         m_size;
    std::unique_ptr<ValueRef<std::string>>    m_name;
    std::vector<std::unique_ptr<Effect>>                m_effects_to_apply_after;
};
...
/** Creates a new system with the specified \a colour and at the specified
  * location. */
class FO_COMMON_API focs::CreateSystem final : public Effect {
public:
    CreateSystem(std::unique_ptr<ValueRef< ::StarType>>&& type,
                 std::unique_ptr<ValueRef<double>>&& x,
                 std::unique_ptr<ValueRef<double>>&& y,
                 std::unique_ptr<ValueRef<std::string>>&& name,
                 std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);

    CreateSystem(std::unique_ptr<ValueRef<double>>&& x,
                 std::unique_ptr<ValueRef<double>>&& y,
                 std::unique_ptr<ValueRef<std::string>>&& name,
                 std::vector<std::unique_ptr<Effect>>&& effects_to_apply_after);
...
private:
    std::unique_ptr<ValueRef< ::StarType>>    m_type;
    std::unique_ptr<ValueRef<double>>         m_x;
    std::unique_ptr<ValueRef<double>>         m_y;
    std::unique_ptr<ValueRef<std::string>>    m_name;
    std::vector<std::unique_ptr<Effect>>                m_effects_to_apply_after;
...
class FO_COMMON_API focs::Destroy final : public Effect {
public:
    Destroy();
...
class FO_COMMON_API focs::AddSpecial final : public Effect {
public:
    explicit AddSpecial(std::string& name, float capacity = 1.0f);
    explicit AddSpecial(std::unique_ptr<ValueRef<std::string>>&& name,
                        std::unique_ptr<ValueRef<double>>&& capacity = nullptr);

    void Execute(ScriptingContext& context) const override;

    std::string Dump(unsigned short ntabs = 0) const override;
    void SetTopLevelContent(const std::string& content_name) override;
    const ValueRef<std::string>* GetSpecialName() const { return m_name.get(); }
    unsigned int GetCheckSum() const override;

private:
    std::unique_ptr<ValueRef<std::string>> m_name;
    std::unique_ptr<ValueRef<double>> m_capacity;
};
...
class FO_COMMON_API focs::RemoveSpecial final : public Effect {
public:
    explicit RemoveSpecial(std::string& name);
    explicit RemoveSpecial(std::unique_ptr<ValueRef<std::string>>&& name);
...
class FO_COMMON_API focs::AddStarlanes final : public Effect {
public:
    explicit AddStarlanes(std::unique_ptr<Condition>&& other_lane_endpoint_condition);
...
private:
    std::unique_ptr<Condition> m_other_lane_endpoint_condition;
};

/** Removes starlane(s) between the target system and systems that match
  * \a other_lane_endpoint_condition */
class FO_COMMON_API focs::RemoveStarlanes final : public Effect {
public:
    explicit RemoveStarlanes(std::unique_ptr<Condition>&& other_lane_endpoint_condition);
...
/** Sets the star type of the target to \a type.  This has no effect on
  * non-System targets. */
class FO_COMMON_API focs::SetStarType final : public Effect {
public:
    explicit SetStarType(std::unique_ptr<ValueRef<::StarType>>&& type);
...
/** Moves an UniverseObject to a location of another UniverseObject that matches
  ...
  * nothing is done. */
class FO_COMMON_API focs::MoveTo final : public Effect {
public:
    explicit MoveTo(std::unique_ptr<Condition>&& location_condition);
...
private:
    std::unique_ptr<Condition> m_location_condition;
};
...
/** Moves an UniverseObject to a location as though it was moving in orbit of
  ...
  * rotation.*/
class FO_COMMON_API focs::MoveInOrbit final : public Effect {
public:
    MoveInOrbit(std::unique_ptr<ValueRef<double>>&& speed,
                std::unique_ptr<Condition>&& focal_point_condition);
    MoveInOrbit(std::unique_ptr<ValueRef<double>>&& speed,
                std::unique_ptr<ValueRef<double>>&& focus_x = nullptr,
                std::unique_ptr<ValueRef<double>>&& focus_y = nullptr);
...
private:
    std::unique_ptr<ValueRef<double>> m_speed;
    std::unique_ptr<Condition>       m_focal_point_condition;
    std::unique_ptr<ValueRef<double>> m_focus_x;
    std::unique_ptr<ValueRef<double>> m_focus_y;
};

/** Moves an UniverseObject a specified distance towards some object or
  * position on the map. */
class FO_COMMON_API focs::MoveTowards final : public Effect {
public:
    MoveTowards(std::unique_ptr<ValueRef<double>>&& speed,
                std::unique_ptr<Condition>&& dest_condition);
    MoveTowards(std::unique_ptr<ValueRef<double>>&& speed,
                std::unique_ptr<ValueRef<double>>&& dest_x = nullptr,
                std::unique_ptr<ValueRef<double>>&& dest_y = nullptr);
...
private:
    std::unique_ptr<ValueRef<double>> m_speed;
    std::unique_ptr<Condition>       m_dest_condition;
    std::unique_ptr<ValueRef<double>> m_dest_x;
    std::unique_ptr<ValueRef<double>> m_dest_y;
};
...
/** Sets the route of the target fleet to move to an UniverseObject that
  ...
  * condition, then one is chosen.  If no objects match the condition, then
  * nothing is done. */
class FO_COMMON_API focs::SetDestination final : public Effect {
public:
    explicit SetDestination(std::unique_ptr<Condition>&& location_condition);
...
class FO_COMMON_API focs::SetAggression final : public Effect {
public:
...
class FO_COMMON_API focs::Victory final : public Effect {
public:
...
class FO_COMMON_API focs::SetEmpireTechProgress final : public Effect {
public:
    SetEmpireTechProgress(std::unique_ptr<ValueRef<std::string>>&& tech_name,
                          std::unique_ptr<ValueRef<double>>&& research_progress,
...
private:
    std::unique_ptr<ValueRef<std::string>>    m_tech_name;
    std::unique_ptr<ValueRef<double>>         m_research_progress;
    std::unique_ptr<ValueRef<int>>            m_empire_id;
};

class FO_COMMON_API focs::GiveEmpireTech final : public Effect {
public:
    explicit GiveEmpireTech(std::unique_ptr<ValueRef<std::string>>&& tech_name,
                            std::unique_ptr<ValueRef<int>>&& empire_id = nullptr);
...
class FO_COMMON_API focs::GenerateSitRepMessage final : public Effect {
public:
    using MessageParams =  std::vector<std::pair<
        std::string, std::unique_ptr<ValueRef<std::string>>>>;

    GenerateSitRepMessage(std::string& message_string, std::string& icon,
                          MessageParams&& message_parameters,
                          std::unique_ptr<ValueRef<int>>&& recipient_empire_id,
    ...
    std::vector<std::pair<std::string, ValueRef<std::string>* >> MessageParameters() const;

    ValueRef<int>*    RecipientID() const     { return m_recipient_empire_id.get(); }
    Condition*       GetCondition() const    { return m_condition.get(); }
...
private:
    ...
    std::string             m_icon;
    std::vector<std::pair<std::string, std::unique_ptr<ValueRef<std::string>>>>
                            m_message_parameters;
    std::unique_ptr<ValueRef<int>>
...
class FO_COMMON_API focs::SetOverlayTexture final : public Effect {
public:
    SetOverlayTexture(std::string& texture, std::unique_ptr<ValueRef<double>>&& size);
    SetOverlayTexture(std::string& texture, ValueRef<double>* size);
...
class FO_COMMON_API focs::SetTexture final : public Effect {
public:
...
class FO_COMMON_API focs::SetVisibility final : public Effect {
public:
    SetVisibility(std::unique_ptr<ValueRef<Visibility>> vis,
                  EmpireAffiliationType affiliation,
                  std::unique_ptr<ValueRef<int>>&& empire_id = nullptr,
                  std::unique_ptr<Condition>&& of_objects = nullptr);    // if not specified, acts on target. if specified, acts on all matching objects

    void Execute(ScriptingContext& context) const override;
    std::string Dump(unsigned short ntabs = 0) const override;
    void SetTopLevelContent(const std::string& content_name) override;

    ValueRef<Visibility>* GetVisibility() const
    { return m_vis.get(); }

    ValueRef<int>* EmpireID() const
    { return m_empire_id.get(); }

    EmpireAffiliationType Affiliation() const
    { return m_affiliation; }

    Condition* OfObjectsCondition() const
    { return m_condition.get(); }

    unsigned int GetCheckSum() const override;

private:
    std::unique_ptr<ValueRef<Visibility>> m_vis;
    std::unique_ptr<ValueRef<int>> m_empire_id;
    EmpireAffiliationType m_affiliation;
...
class FO_COMMON_API focs::Conditional final : public Effect {
public:
    Conditional(std::unique_ptr<Condition>&& target_condition,
...
private:
    std::unique_ptr<Condition> m_target_condition; // condition to apply to each target object to determine which effects to execute
