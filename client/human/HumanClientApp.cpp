...
namespace {
    ...
    void SetGLVersionDependentOptionDefaults() {
        ...
        if (!GetOptionsDB().Get<bool>("gl_version.check.enabled"))
            return;
        GetOptionsDB().Set<bool>("gl_version.check.enabled", false);
    ...
    m_game_saves_in_progress()
{
    ...
    GetOptionsDB().OptionChangedSignal("content.path").connect(
...
void HumanClientApp::StartServer() {
    ...
    args.push_back("--content.path");
    args.push_back("\"" + GetOptionsDB().Get<std::string>("content.path") + "\"");
