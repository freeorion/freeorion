...
bool ClientNetworking::ConnectToServer(const std::string& ip_address, boost::posix_time::seconds timeout/* = boost::posix_time::seconds(5)*/)
{
    using namespace boost::asio::ip;
...
bool ClientNetworking::ConnectToLocalHostServer(boost::posix_time::seconds timeout/* = boost::posix_time::seconds(5)*/)
    ...
    if (error) {
        ...
        if (bytes_transferred == HEADER_SIZE) {
            ...
            boost::asio::async_read(m_socket, boost::asio::buffer(m_incoming_message.Data(), m_incoming_message.Size()),
                                    boost::bind(&ClientNetworking::HandleMessageBodyRead, this,
                                                boost::asio::placeholders::error,
                                                boost::asio::placeholders::bytes_transferred));
        }
    }
...
void ClientNetworking::AsyncWriteMessage()
{
    ...
    buffers.push_back(boost::asio::buffer(m_outgoing_messages.front().Data(), m_outgoing_messages.front().Size()));
