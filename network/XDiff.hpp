...
/** computes the difference between two XML documents.  The result of this computation reflects the changes that must be 
   ...
   any attributes not in the original element should be added to it.
   - \verbatim "REDOATTR " \endverbatim
   This element's attributes should replace all the attributes in the original element.  This is necessary when there are some
   attributes that must be deleted, and some that must be updated or added.  Since it is not possible to annotate individual
...
{
public:
   /** ctor that takes two input XML files and writes the result of the xdiff operation to a third. */
   XDiff(const std::string& input_file_1, const std::string& input_file_2, const std::string& output_file); 
...
private:
   ...
   void writeDiff(const std::string& input_file, const std::string& output_file);
   void writeDocDiff(std::stringstream& out, bool whitespace = false);
   void writeDeleteNode(std::ostream &out, int node, int indent, bool whitespace = false);
   void writeInsertNode(std::ostream &out, int node, int indent, bool whitespace = false);
   void writeMatchNode(std::ostream &out, XTree *xtree, int node, int indent, bool whitespace = false);
   void writeDiffNode(std::ostream &out, int node1, int node2, int indent, bool whitespace = false);
