...
public:
    ...
    [[nodiscard]] float       GroupOutput(UniverseObjectID object_id) const;    ///< returns amount of resource being generated by resource sharing group that contains the object with id \a object_id
    ...
    [[nodiscard]] float       GroupTargetOutput(UniverseObjectID object_id) const;
    ...
    [[nodiscard]] float       GroupAvailable(UniverseObjectID object_id) const; ///< returns amount of resource available in resource sharing group that contains the object with id \a object_id
    ...
    void SetObjects(std::vector<UniverseObjectID> object_ids) noexcept;

    /** specifies which sets systems can share resources.  any two sets should
      * have no common systems. */
    void SetConnectedSupplyGroups(std::set<std::set<UniverseObjectID>> connected_system_groups) noexcept;
...
private:
    ...
    std::vector<UniverseObjectID>        m_object_ids;                                       ///< IDs of objects to consider in this pool
    std::set<std::set<UniverseObjectID>> m_connected_system_groups;                          ///< sets of systems between and in which objects can share this pool's resource
    std::map<id_flat_set, float>         m_connected_object_groups_resource_output;          ///< cached map from connected group of objects that can share resources, to how much resource is output by ResourceCenters in the group.  regenerated during update from other state information. // TODO: flat_map ?
    std::map<id_flat_set, float>         m_connected_object_groups_resource_target_output;   ///< cached map from connected group of objects that can share resources, to how much resource would, if all meters equaled their target meters, be output by ResourceCenters in the group.  regenerated during update from other state information.
