...
public:
    using container_type = std::map<EmpireID, std::shared_ptr<Empire>>;
    using iterator = container_type::iterator;
    using const_container_type = std::map<EmpireID, std::shared_ptr<const Empire>>;
    ...
    [[nodiscard]] std::shared_ptr<const Empire>         GetEmpire(EmpireID id) const;  //!< Returns the empire whose ID is \a id, or nullptr if none exist
    [[nodiscard]] std::shared_ptr<const UniverseObject> GetSource(EmpireID id, const ObjectMap& objects) const;  //!< Return the empire source or nullptr if the empire or source doesn't exist
    ...
    [[nodiscard]] DiplomaticStatus          GetDiplomaticStatus(EmpireID empire1, EmpireID empire2) const;

    [[nodiscard]] boost::container::flat_set<EmpireID> GetEmpireIDsWithDiplomaticStatusWithEmpire(
    ...
    [[nodiscard]] static boost::container::flat_set<EmpireID> GetEmpireIDsWithDiplomaticStatusWithEmpire(
        EmpireID empire_id, DiplomaticStatus diplo_status, const DiploStatusMap& statuses);

    [[nodiscard]] bool                      DiplomaticMessageAvailable(EmpireID sender_id, EmpireID recipient_id) const;
    [[nodiscard]] const DiplomaticMessage&  GetDiplomaticMessage(EmpireID sender_id, EmpireID recipient_id) const;

    [[nodiscard]] std::string               Dump() const;

    [[nodiscard]] std::shared_ptr<Empire>   GetEmpire(EmpireID id);  //!< Returns the empire whose ID is \a id, or nullptr if none exist
...
private:
    ...
    void GetDiplomaticMessagesToSerialize(std::map<std::pair<EmpireID, EmpireID>, DiplomaticMessage>& messages,
    ...
    std::map<std::pair<EmpireID, EmpireID>, DiplomaticMessage> m_diplomatic_messages;
