...
public:
    ...
    [[nodiscard]] const std::set<std::pair<UniverseObjectID, UniverseObjectID>>& SupplyStarlaneTraversals(EmpireID empire_id) const;
    ...
    [[nodiscard]] const std::set<std::pair<UniverseObjectID, UniverseObjectID>>& SupplyObstructedStarlaneTraversals(EmpireID empire_id) const;
    ...
    [[nodiscard]] const std::set<UniverseObjectID>& FleetSupplyableSystemIDs(EmpireID empire_id) const;
    [[nodiscard]] std::vector<UniverseObjectID>     FleetSupplyableSystemIDs(EmpireID empire_id, bool include_allies, const ScriptingContext& context) const;
    [[nodiscard]] EmpireID                          EmpireThatCanSupplyAt(UniverseObjectID system_id) const;
    ...
    [[nodiscard]] const std::set<std::set<UniverseObjectID>>& ResourceSupplyGroups(EmpireID empire_id) const;
    ...
    [[nodiscard]] const std::map<UniverseObjectID, float>& PropagatedSupplyRanges(EmpireID empire_id) const;
    ...
    /** Returns the distance from each system for the empire with id
      ...
      * through obstructed systems for that empire. */
    [[nodiscard]] const std::map<UniverseObjectID, float>& PropagatedSupplyDistances(EmpireID empire_id) const;
    ...
    [[nodiscard]] bool SystemHasFleetSupply(UniverseObjectID system_id, EmpireID empire_id) const;
    [[nodiscard]] bool SystemHasFleetSupply(UniverseObjectID system_id, EmpireID empire_id, bool include_allies,
...
private:
    ...
    std::map<EmpireID, std::set<std::pair<UniverseObjectID, UniverseObjectID>>> m_supply_starlane_traversals;
    ...
    std::map<EmpireID, std::set<std::pair<UniverseObjectID, UniverseObjectID>>> m_supply_starlane_obstructed_traversals;

    /** ids of systems where fleets can be resupplied. indexed by empire id. */
    std::map<EmpireID, std::set<UniverseObjectID>> m_fleet_supplyable_system_ids;
    ...
    /** sets of system ids that are connected by supply lines and are able to
        ...
        by empire id. */
    std::map<EmpireID, std::set<std::set<UniverseObjectID>>> m_resource_supply_groups;
    ...
    std::map<UniverseObjectID, float> m_propagated_supply_ranges;
    ...
    std::map<EmpireID, std::map<UniverseObjectID, float>> m_empire_propagated_supply_ranges;
    ...
    std::map<UniverseObjectID, float> m_propagated_supply_distances;

    /** for each empire, what systems it can propagate supply into, and how far
      * that system is from the closest source of supply for that empire, along
      * possible supply propgation connections (ie. not though a supply
      * obstructed system) */
    std::map<EmpireID, std::map<UniverseObjectID, float>> m_empire_propagated_supply_distances;
