...
private:
    ...
    std::string::const_iterator ParseTagsImpl(const std::string::const_iterator& start,
        ...
        const static regex tag("<(?<begin_tag>\\w+)( (?<params>[^>]+))?>|</(?<end_tag>\\w+)>");
        ...
        while(boost::regex_search(current, end, match, tag, flags)) {
            ...
            if(begin_match.matched) {
                ...
                if(tags && current != current + match.position()) {
                    tags->push_back(RichTextTag(RichText::PLAINTEXT_TAG, "", std::string(current, current + match.position())));
                }

                // Recurse to the next nesting level.
                current = FinishTag(begin_match, match["params"], current + match.position() + match.length(), end, tags);
        ...
        if(tags && current != end) {
            tags->push_back(RichTextTag(RichText::PLAINTEXT_TAG, "", std::string(current, end)));
        }
     ...
     */
    std::basic_string< char >::const_iterator FinishTag(const std::string& tag,
        ...
        if (current == end) {
            ...
            if (StartsWith(current, end, end_tag)) {
                ...
                if(tags != 0) {
                    tags->push_back(RichTextTag(tag, parameters, std::string(start, current)));
                }
    ...
    RichTextTag CreateErrorTag(const char* what) {
        return RichTextTag( RichText::PLAINTEXT_TAG, "", std::string("<rgba 255 0 0 255>") + what + "</rgba>" );
    }
