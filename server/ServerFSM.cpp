...
boost::statechart::result Idle::react(const HostMPGame& msg)
{
    ...
    if (server.m_networking.empty() && server.m_expected_ai_players.empty()) { // TODO: superfluous
        ...
        server.m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Server now in mode " << SERVER_MP_LOBBY << " (SERVER_MP_LOBBY).";
        ...
        server.m_log_category.errorStream() << "ServerApp::HandleNonPlayerMessage : A human player attempted to host "
...
boost::statechart::result Idle::react(const HostSPGame& msg)
{
    ...
    if (server.m_networking.empty() && server.m_expected_ai_players.empty()) { // TODO: superfluous
        ...
        player_connection->SendMessage(JoinAckMessage(player_id));
        server.m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Server now in mode " << SERVER_GAME_SETUP << " (SERVER_GAME_SETUP).";
        server.m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Universe size set to " << server.m_galaxy_size << " systems (SERVER_GAME_SETUP).";
        server.m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Universe shape set to " << server.m_galaxy_shape << " (SERVER_GAME_SETUP).";
    } else {
        server.m_log_category.errorStream() << "ServerApp::HandleNonPlayerMessage : A human player attempted to host "
            "a new game but there was already one in progress or one being setup.  Terminating connection.";
        server.m_networking.Disconnect(player_connection);
...
boost::statechart::result MPLobby::react(const StartMPGame& msg)
{
    ...
    if (server.m_lobby_data.m_new_game) { // new game
        ...
        server.m_player_save_game_data.clear();
        server.m_log_category.debugStream() << "ServerApp::HandleMessage : Server now in mode " << SERVER_GAME_SETUP << " (SERVER_GAME_SETUP).";
        if (server.m_expected_players == static_cast<int>(server.m_networking.size())) {
            server.NewGameInit();
            server.m_state = SERVER_WAITING;
            server.m_log_category.debugStream() << "ServerApp::HandleMessage : Server now in mode " << SERVER_WAITING << " (SERVER_WAITING).";
        }
    } else { // load game
        ...
        if (sender_it != server.m_networking.end() && (*sender_it)->Host()) {
            ...
            server.m_log_category.errorStream() << "Player #" << message.SendingPlayer() << " attempted to initiate a game load, but is not the host, or is "
                "not found in the player list."; // TODO: Remove part about not being in player list.
        }
    }
...
boost::statechart::result WaitingForJoiners::react(const Disconnection& d)
{
    PlayerConnectionPtr& player_connection = d.m_player_connection;
    return forward_event();
}
...
boost::statechart::result WaitingForJoiners::react(const JoinGame& msg)
{
    ...
    if (it != server.m_expected_ai_players.end()) { // incoming AI player connection
        ...
        if (static_cast<int>(server.m_expected_ai_players.size() + server.m_networking.size()) < server.m_expected_players) {
            ...
            server.m_log_category.errorStream() << "ServerApp::HandleNonPlayerMessage : A human player attempted to join "
    ...
    if (static_cast<int>(server.m_networking.size()) == server.m_expected_players) { // if we've gotten all the players joined up
        if (server.m_player_save_game_data.empty())
            ...
            server.LoadGameInit();
        server.m_state = SERVER_WAITING;
        server.m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Server now in mode " << SERVER_WAITING << " (SERVER_WAITING).";
    }
...
boost::statechart::result WaitingForTurnEnd::react(const Disconnection& d)
{
    PlayerConnectionPtr& player_connection = d.m_player_connection;
    return forward_event();
}

boost::statechart::result WaitingForTurnEnd::react(const SaveGameRequest& msg)
{
    ServerApp& server = context<ServerFSM>().Server();
    const Message& message = msg.m_message;
    ...
    if (server.m_networking.GetPlayer(message.SendingPlayer()) != server.m_networking.end()) { // TODO: superfluous
        ...
        server.m_log_category.errorStream() << "Player #" << message.SendingPlayer() << " attempted to initiate a game save, but is not found in the player list.";
    }
...
boost::statechart::result WaitingForTurnEnd::react(const LoadSPGame& msg)
{
    ...
    if (sender_it != server.m_networking.end() && (*sender_it)->Host()) {
        ...
        server.m_log_category.errorStream() << "Player #" << message.SendingPlayer() << " attempted to initiate a game save, but is not the host, or is "
            "not found in the player list."; // TODO Remove part about player list.
    }
...
boost::statechart::result WaitingForTurnEnd::react(const TurnOrders& msg)
{
    ...
    server.m_log_category.debugStream() << "ServerApp::HandleMessage : Received orders from player " << message.SendingPlayer();

    /* if all orders are received already, do nothing as we are processing a turn */
    if (server.AllOrdersReceived())
        return forward_event(); // TODO: Huh?
    ...
    if (server.AllOrdersReceived()) {
        server.m_log_category.debugStream() << "ServerApp::HandleMessage : All orders received; processing turn...";
        server.ProcessTurns();
    }
    ...
    if (colon_position != std::string::npos) {
        ...
        for (unsigned int i = 0; i < tokens.size(); ++i) {
            ...
            if (token_is_name)
                ...
                return forward_event(); // TODO: What to return here?
        }
    }
...
boost::statechart::result WaitingForTurnEnd::react(const EndGame& msg)
{
    ...
    if (it != server.m_networking.end() && (*it)->Host()) {
        ...
        server.m_log_category.debugStream() << "ServerApp::HandleMessage : Server now in mode " << SERVER_DYING << " (SERVER_DYING).";
...
boost::statechart::result WaitingForSaveData::react(const Disconnection& d)
{
    PlayerConnectionPtr& player_connection = d.m_player_connection;
    return forward_event();
}
...
boost::statechart::result WaitingForSaveData::react(const SaveGameRequest& msg)
{ return defer_event(); }

boost::statechart::result WaitingForSaveData::react(const LoadSPGame& msg)
{ return defer_event(); }

boost::statechart::result WaitingForSaveData::react(const TurnOrders& msg)
{ return defer_event(); }

boost::statechart::result WaitingForSaveData::react(const RequestObjectID& msg)
{ return defer_event(); }

boost::statechart::result WaitingForSaveData::react(const PlayerChat& msg)
{ return defer_event(); }

boost::statechart::result WaitingForSaveData::react(const EndGame& msg)
{ return defer_event(); }
