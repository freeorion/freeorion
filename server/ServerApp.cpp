...
void ServerApp::HandleNonPlayerMessage(const Message& msg, const ServerNetworkCore::ConnectionInfo& connection)
{
    ...
    case Message::HOST_GAME: {
        if (m_players_info.empty() && m_expected_ai_players.empty()) {
            ...
            m_universe_shape = ClientUniverse::Shape(boost::lexical_cast<int>(doc.root_node.Child("universe_params").Attribute("shape")));
            if (m_universe_shape == ClientUniverse::FROM_FILE)
                m_universe_file = doc.root_node.Child("universe_params").Child("file").Text();
            ...
            m_log_category.debugStream() << "ServerApp::HandleNonPlayerMessage : Universe size set to " << m_universe_size << " systems (SERVER_GAME_SETUP).";
    ...
    case Message::JOIN_GAME: {
        ...
        if (static_cast<int>(m_players_info.size()) == m_expected_players) { // if we've gotten all the players joined up
            GameInit();
...
void ServerApp::GameInit()
{
    ...
    for (unsigned int i = 0; i < m_players_info.size(); ++i) {
        GG::XMLDoc doc;
#if 0
        doc.root_node.AppendChild(m_universe.XMLEncode(i));
