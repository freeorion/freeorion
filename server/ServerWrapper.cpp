...
namespace {
    ...
    list FilterIDsWithCondition(const focs::Condition* cond, const list &obj_ids) {
        list permitted_ids;
        ...
        if (cond && cond->SourceInvariant())
            cond->Eval(focs::ScriptingContext(), permitted_objs, objs);
        else
