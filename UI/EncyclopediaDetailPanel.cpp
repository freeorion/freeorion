...
namespace {
    ...
    void SearchPediaArticleTextForWords(    const std::string& article_key,
                                            const std::string& article_directory,
                                            ...
                                            std::size_t idx)
    {
        ...
        const bool all_handled = CustomToLower(desc_lower.begin(), desc_lower.end());
        if (!all_handled)
                                            ...
                                            bool search_article_text)
    {
        ...
        if (search_article_text)
            SearchPediaArticleTextForWords(article_key, article_directory, article_name_link,
                                           article_match, search_text, words_in_search_text, idx);
    }
}
...
void EncyclopediaDetailPanel::HandleSearchTextEntered() {
    ...
    if (search_desc) {
        timer.EnterSection("non-parallel article text search");
        ...
        for (auto& [article_key_directory, article_name_link] : pedia_entries) {
            ...
            idx++;
            auto& article_match{article_match_report[idx]};
            auto& [article_key, article_directory] = article_key_directory;

            SearchPediaArticleTextForWords(article_key, article_directory, std::move(article_name_link),
                                           article_match, search_text, words_in_search_text, idx);
        }
    }
