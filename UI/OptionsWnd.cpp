...
void OptionsWnd::CompleteConstruction() {
    ...
    SoundFileOption(current_page, 1, "audio.effects.alert.path", UserString("OPTIONS_SOUND_ALERT"));
    ...
    auto row = GG::Wnd::Create<OptionsListRow>(ROW_WIDTH, window_reset_button->MinUsableSize().y + LAYOUT_MARGIN + 6,
    ...
    row = GG::Wnd::Create<OptionsListRow>(ROW_WIDTH, flush_button->MinUsableSize().y + LAYOUT_MARGIN + 6, flush_button, 0);
    current_page->Insert(row);
    ...
    row = GG::Wnd::Create<OptionsListRow>(ROW_WIDTH, show_font_texture_button ->MinUsableSize().y + LAYOUT_MARGIN + 6, show_font_texture_button , 0);
    current_page->Insert(row);
    ...
    BoolOption(current_page,   0, "ui.window.queue.production_location.shown", UserString("OPTIONS_UI_PROD_QUEUE_LOCATION"));
    ...
    ColorOption(current_page, 0, "ui.control.button.state.color", UserString("OPTIONS_STATE_BUTTON_COLOR"));
    ...
    BoolOption(current_page, 0, "save.auto.single_player.start.enabled",
    ...
    BoolOption(current_page, 0, "content.shipdesign.saved.enabled",    UserString("OPTIONS_ADD_SAVED_DESIGNS"));
...
void OptionsWnd::ResolutionOption(GG::ListBox* page, int indentation_level) {
    ...
    if (GG::SDLGUI::GetGUI()->FramebuffersAvailable()) {
        BoolOption(page, indentation_level, "video.fake_mode.enabled", UserString("OPTIONS_FAKE_MODE_CHANGE"));
    } else {
