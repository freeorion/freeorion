...
private:
    ...
    std::vector<std::pair<MeterType, MeterType>>    m_meter_types;      // list of <MeterType, MeterType> pairs, where .first is the "actual" meter value, and .second is the the "target" or "max" meter value.  Either may be MeterType::INVALID_METER_TYPE, in which case nothing will be shown for that MeterType
