...
namespace {
    ...
    void AddOptions(OptionsDB& db) {
        ...
        db.Add("setup.empire.color",            UserStringNop("OPTIONS_DB_GAMESETUP_EMPIRE_COLOR"),            9,                  RangedValidator<int>(0, 100));
...
void GalaxySetupWnd::CompleteConstruction() {
    ...
    m_empire_color_label->SetBrowseText(UserString(GetOptionsDB().GetDescription("setup.empire.color")));
    m_empire_color_selector = GG::Wnd::Create<EmpireColorSelector>(CONTROL_HEIGHT - CONTROL_VMARGIN);
    m_empire_color_selector->Select(GetOptionsDB().Get<int>("setup.empire.color"));
