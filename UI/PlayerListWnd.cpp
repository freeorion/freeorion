...
void PlayerListWnd::PlayerRightClicked(GG::ListBox::iterator it, const GG::Pt& pt, const GG::Flags<GG::ModKey>& modkeys) {
        ...
        clicked_empire_id != ALL_EMPIRES)
    {
        ...
        if (show_peace_propose)
            popup.AddMenuItem(GG::MenuItem(UserString("PEACE_PROPOSAL"),           2, false, false, peace_proposal_action));
        if (show_peace_cancel)
            popup.AddMenuItem(GG::MenuItem(UserString("PEACE_PROPOSAL_CANCEL"),    4, false, false, proposal_cancel_action));
        if (show_peace_accept)
            popup.AddMenuItem(GG::MenuItem(UserString("PEACE_ACCEPT"),             3, false, false, peace_accept_action));
        if (show_peace_reject)
            popup.AddMenuItem(GG::MenuItem(UserString("PEACE_REJECT"),             9, false, false, proposal_reject_action));
        if (show_allies_end)
            popup.AddMenuItem(GG::MenuItem(UserString("END_ALLIANCE_DECLARATION"), 8, false, false, end_alliance_declaration_action));
        if (show_allies_propose)
            popup.AddMenuItem(GG::MenuItem(UserString("ALLIES_PROPOSAL"),          6, false, false, allies_proposal_action));
        if (show_allies_accept)
            popup.AddMenuItem(GG::MenuItem(UserString("ALLIES_ACCEPT"),            7, false, false, allies_accept_action));
        if (show_allies_cancel)
            popup.AddMenuItem(GG::MenuItem(UserString("ALLIES_PROPOSAL_CANCEL"),   4, false, false, proposal_cancel_action));
        if (show_allies_reject)
            popup.AddMenuItem(GG::MenuItem(UserString("ALLIES_REJECT"),            9, false, false, proposal_reject_action));
        if (show_declare_war)
            popup.AddMenuItem(GG::MenuItem(UserString("WAR_DECLARATION"),          1, false, false, war_declaration_action));
    }

    popup.AddMenuItem(GG::MenuItem(str(FlexibleFormat(UserString("ENC_LOOKUP")) % GetEmpire(clicked_empire_id)->Name()), 5, false, false, pedia_lookup_action));
