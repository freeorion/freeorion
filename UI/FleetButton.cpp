...
namespace {
    ...
    void AddOptions(OptionsDB& db) {
        db.Add("ui.window.map.fleet.selection_indicator.size", UserStringNop("OPTIONS_DB_UI_FLEET_SELECTION_INDICATOR_SIZE"), 1.625, RangedStepValidator<double>(0.125, 0.5, 5));
    }
...
void FleetButton::LayoutIcons() {
    ...
    if (m_selection_indicator) {
        ...
        //GG::Pt subtexture_sz = GG::Pt(subtexture.Width(), subtexture.Height());
        double sel_ind_scale = GetOptionsDB().Get<double>("ui.window.map.fleet.selection_indicator.size");
...
{ Sound::GetSound().PlaySound(GetOptionsDB().Get<std::string>("ui.window.map.fleet.button.rollover.sound.path"), true); }

void FleetButton::PlayFleetButtonOpenSound()
{ Sound::GetSound().PlaySound(GetOptionsDB().Get<std::string>("ui.window.map.fleet.button.click.sound.path"), true); }
