...
public: 
		...
		NodeInfo(OrthoRep& H, GridLayout& L, node v, adjEntry adj, RoutingChannel<int>& rc,
                 NodeArray<int>& nw, NodeArray<int>& nh) : m_adj(adj)
		        { init(); get_data(H, L, v, rc, nw, nh); }
		virtual ~NodeInfo() {};

		adjEntry cage_entry() {return m_adj;}
		
          //get
		int coord(OrthoDir bs) const {return m_coord[bs];}//nodeboxside coordinates (real size)
        int cage_coord(OrthoDir bs) const {return m_ccoord[bs];}//nodecageside coordinates (expanded size)
        int cageCoord(OrthoDir bs) const {return m_ccoord[bs];}//nodecageside coordinates (expanded size)
		...
		int rc(OrthoDir od) const {return m_rc[od];} //routing channel size

		List<edge>& inList(OrthoDir bs) {return in_edges[bs];}
		List<bool>& inPoint(OrthoDir bs) {return point_in[bs];}
		...
		int delta(OrthoDir mainside, OrthoDir neighbour) const {return m_delta[mainside][neighbour];}
		
		  //paper epsilon
		int eps(OrthoDir mainside, OrthoDir neighbour) const {return m_eps[mainside][neighbour];}
        ...
        int& flips(OrthoDir s1, OrthoDir s2) {return m_flips[s1][s2];} 

		int num_bend_free(OrthoDir s)  const {return m_nbf[s];} //number of edges routed bendfree
                int& nbf(OrthoDir s) {return m_nbf[s];}
        
		int num_edges(OrthoDir od) const {return num_s_edges[od]; //return number of edges at side od
		}
		  //position of gen. edges in edge lists for every side, starting with 1
		int gen_pos(OrthoDir od) const {return m_gen_pos[od];}
		bool has_gen(OrthoDir od) {return m_gen_pos[od] > -1;}
        ...
        void set_coord(OrthoDir bs, int co) {m_coord[bs] = co;}
        void set_cage_coord(OrthoDir bs, int co) {m_ccoord[bs] = co;}
        void setCageCoord(OrthoDir bs, int co) {m_ccoord[bs] = co;}
		...
		{num_s_edges[od] = num;} //odir: N 0, E 1, check correct od parameter?
		  ...
		  //computes the size of the cage face and the node box
                void compute_cage_size() {cage_x_size = m_ccoord[odSouth]-m_ccoord[odNorth]; 
...
//		int compute_rc(box_side b) {cout<<"rc not yet implemented\n";exit(1);}
		...
		void inc_E_hook(OrthoDir s_from, OrthoDir s_to, int num = 1)  //paper set E_s1_s2
		{m_routable[s_from][s_to] += num; m_nbe[s_from][s_to] += num;}
        ...
        {m_nbe[s_from][s_to] += num;}
		  ...
		  //(needs positions ...)
		void get_data(OrthoRep& O, GridLayout& L, node v, RoutingChannel<int>& rc,
                      NodeArray<int>& nw, NodeArray<int>& nh); //check input parameter
		void get_OR_data(node v, OrthoRep& O);
          //
		int num_routable(OrthoDir s_from, OrthoDir s_to) const {return m_routable[s_from][s_to];} //card. of paper E^_s1,s2
        int& numr(OrthoDir s_from, OrthoDir s_to) {return m_routable[s_from][s_to];}
