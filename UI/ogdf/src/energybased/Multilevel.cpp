...
inline DPoint Multilevel :: get_waggled_inbetween_position(DPoint s, DPoint t,double 
							   lambda)
{ 
  mathExtension M;
  ...
  double radius = WAGGLEFACTOR * M.norm(t-s);
  double rnd = double(randomNumber(1,BILLION)+1)/(BILLION+2);//rand number in (0,1)
...
inline DPoint Multilevel :: calculate_position(DPoint P,DPoint Q, double dist_P,
					       double dist_Q)
{
  mathExtension M;
  double dist_PQ = M.norm(P-Q);
