...
void FMMMLayout :: call(GraphAttributes &AG, const EdgeArray<double> &edgeLength,
			char* ps_file)
{
	call(AG,edgeLength);
	create_postscript_drawing(AG,ps_file);
...
void FMMMLayout :: call_DIVIDE_ET_IMPERA_step(Graph& G,NodeArray<NodeAttributes>& A,
...
inline void FMMMLayout::call_POSTPROCESSING_step(
	Graph& G,
...
inline void FMMMLayout :: import_EdgeAttributes(const Graph& G, const EdgeArray<double>& 
...
inline void FMMMLayout :: init_ind_ideal_edgelength(const Graph& G,NodeArray
						    <NodeAttributes>& 
...
inline void FMMMLayout :: set_radii(const Graph& G,NodeArray<NodeAttributes>& A)
{
 node v;
 radius.init(G);
 double w,h;
...
inline void FMMMLayout :: export_NodeAttributes(Graph& G_reduced, NodeArray
						<NodeAttributes>& 
...
void FMMMLayout :: make_simple_loopfree(const Graph& G,NodeArray<NodeAttributes>& 
...
void FMMMLayout :: delete_parallel_edges(const Graph& G,EdgeArray<EdgeAttributes>& E,
...
inline void FMMMLayout :: update_edgelength(List<edge>& S,EdgeArray<double>& 
...
void FMMMLayout :: create_maximum_connected_subGraphs(Graph& G,NodeArray<NodeAttributes>&
...
inline void FMMMLayout :: pack_subGraph_drawings (NodeArray<NodeAttributes>& A,Graph 
						  G_sub[], NodeArray<NodeAttributes> 
						  A_sub[])
{
...
inline void FMMMLayout :: calculate_bounding_rectangles_of_components(List<Rectangle>& R,
...
Rectangle FMMMLayout :: calculate_bounding_rectangle(Graph& G,NodeArray<NodeAttributes>& 
...
void FMMMLayout :: rotate_components_and_calculate_bounding_rectangles(List<Rectangle>&R,
						            Graph G_sub[],NodeArray
							    <NodeAttributes> A_sub[])
{
 int i,j;
 const double PI_half = 1.5707963; //value of PI/2
 double sin_j,cos_j;
 ...
 for(i=0;i<number_of_components;i++)
   {//allcomponents
     ...
     for(j=1;j<=stepsForRotatingComponents();j++)
	  ...
	  angle = PI_half * (double(j)/double(stepsForRotatingComponents()+1));
          sin_j = sin(angle);
          cos_j = cos(angle);
          forall_nodes(v_sub,G_sub[i])
	   {
...
inline double FMMMLayout :: calculate_area(double width,double height,int comp_nr)
{
...
inline void FMMMLayout :: export_node_positions(NodeArray<NodeAttributes>& A,List
						<Rectangle>&  R, Graph G_sub[], 
...
inline void FMMMLayout :: delete_all_subGraphs(Graph G_sub[], NodeArray<NodeAttributes> 
...
inline void FMMMLayout :: calculate_forces(Graph& G, NodeArray<NodeAttributes>& A,
...
void FMMMLayout :: init_boxlength_and_cornercoordinate (Graph& G,NodeArray
							<NodeAttributes>& A)
{
    //boxlength is set 
    ...
    forall_nodes(v,G)
       { 
          w  += M.max(A[v].get_width(),MIN_NODE_SIZE);  
          h  += M.max(A[v].get_height(),MIN_NODE_SIZE);
       }
    
    boxlength = ceil(M.max(w,h) * BOX_SCALING_FACTOR);
...
void FMMMLayout :: create_initial_placement (Graph& G,NodeArray<NodeAttributes>& A) 
{
 ...
 if (initialPlacementForces() == ipfKeepPositions) // don't change anything
   ...
   {//(uniform on a grid)
     init_boxlength_and_cornercoordinate(G,A); 
     mathExtension M;
     int level = static_cast<int>( ceil(M.Log4(G.numberOfNodes())));
...
inline void FMMMLayout:: make_initialisations_for_rep_calc_classes(Graph& G/*,NodeArray
...
inline void FMMMLayout :: calculate_repulsive_forces(Graph &G, NodeArray<NodeAttributes> 
...
inline void FMMMLayout :: deallocate_memory_for_rep_calc_classes()
{
...
void FMMMLayout :: calculate_attractive_forces(Graph& G,NodeArray<NodeAttributes> & A, 
...
void FMMMLayout :: add_attr_rep_forces(Graph& G,NodeArray<DPoint>& F_attr,NodeArray
...
inline void FMMMLayout :: move_nodes(Graph& G,NodeArray<NodeAttributes>& A,NodeArray
			       <DPoint>& F)
{        
 node v;
...
void FMMMLayout :: update_boxlength_and_cornercoordinate(Graph& G,NodeArray
							 <NodeAttributes>&A)
{
 mathExtension M;
 node v;
...
inline double FMMMLayout :: max_radius(int iter)
...
inline void FMMMLayout :: set_average_ideal_edgelength(Graph& G,EdgeArray
						       <EdgeAttributes>& E)
{
...
inline double FMMMLayout :: get_average_forcevector_length (Graph& G,NodeArray<DPoint>& 
							    F)
{
  mathExtension M;
  double lengthsum = 0;
...
void FMMMLayout :: prevent_oscilations(Graph& G, NodeArray<DPoint>& F,NodeArray<DPoint>& 
...
inline void FMMMLayout :: init_last_node_movement(Graph& G, NodeArray<DPoint>& F,
						  NodeArray<DPoint>& last_node_movement)
{
 node v;
 forall_nodes(v,G)
...
void FMMMLayout :: adapt_drawing_to_ideal_average_edgelength(Graph& G,NodeArray
			 <NodeAttributes>& A,EdgeArray<EdgeAttributes>& E)
{
 mathExtension M;
 edge e; 
 node v;
 ...
 forall_edges(e,G)
  {
    ...
    sum_real_edgelength += M.norm(A[e->source()].get_position()-
                                 A[e->target()].get_position());
  }
...
inline void FMMMLayout :: restrict_force_to_comp_box(DPoint& force)
{
...
/*inline clock_t FMMMLayout :: get_time(tms t)
{
  tms now;
  times(&now);
