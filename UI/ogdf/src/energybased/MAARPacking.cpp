...
void MAARPacking :: pack_rectangles_using_Best_Fit_strategy(List<Rectangle>& R,double
							    aspect_ratio, int presort,
...
void MAARPacking :: B_F_insert_rectangle_in_new_row(Rectangle r,List<PackingRowInfo>& P,
					    List <ListIterator<PackingRowInfo> >& 
					    row_of_rectangle, PQueue& total_width_of_row)
{
 PackingRowInfo p;
 mathExtension M; 
 ...
 area_width = M.max(r.get_width(),area_width);
...
void MAARPacking :: B_F_insert_rectangle(Rectangle r,List<PackingRowInfo>& P,List
					 <ListIterator<PackingRowInfo> >& 
					 row_of_rectangle,ListIterator<PackingRowInfo> 
					 B_F_item,PQueue& total_width_of_row)
{
  ...
  else //insert into an existing row
  {
    ...
    p.set_max_height(M.max(old_max_height,r.get_height()));
    ...
    *B_F_item = p;
    ...
    //update area_height,area_width
    area_width = M.max(area_width,p.get_total_width());
    area_height = M.max(area_height,area_height-old_max_height+r.get_height());
...
void MAARPacking :: export_new_rectangle_positions(List<PackingRowInfo>& P,List
						   <ListIterator<PackingRowInfo> >& 
						   row_of_rectangle,
						   List<ListIterator<Rectangle> >& 
						   rectangle_order)
{
 int i;
 Rectangle r;
...
inline double MAARPacking :: calculate_aspect_ratio_area(double width,double height,
							 double aspect_ratio)
{ 
...
bool MAARPacking :: better_tipp_rectangle_in_new_row(Rectangle r,double aspect_ratio,
						     int allow_tipping_over,double&
						     best_area)
{
 ...
 width  = M.max(area_width,r.get_width());
 ...
 if(allow_tipping_over == FMMMLayout::toNoGrowingRow ||
   ...
   {
     width  = M.max(area_width,r.get_height());
...
bool MAARPacking :: better_tipp_rectangle_in_this_row(Rectangle r,double aspect_ratio,
						      int allow_tipping_over,
						      PackingRowInfo B_F_row,double& 
						      best_area)
{
 double height,width,act_area;
 mathExtension M;
 ...
 width  = M.max(area_width,B_F_row.get_total_width()+r.get_width());
 height = M.max(area_height, area_height-B_F_row.get_max_height()+r.get_height());
 best_area   = calculate_aspect_ratio_area(width,height,aspect_ratio);
 ...
 if((allow_tipping_over == FMMMLayout::toNoGrowingRow && r.get_width() 
   ...
   {
     width  = M.max(area_width,B_F_row.get_total_width()+r.get_height());
     height = M.max(area_height, area_height-B_F_row.get_max_height()+r.get_width());
     act_area   = calculate_aspect_ratio_area(width,height,aspect_ratio);
     if(act_area < 0.99999 * best_area)
       {
	 best_area = act_area; 
...
inline Rectangle MAARPacking :: tipp_over(ListIterator<Rectangle>
					  rect_item)
{
  Rectangle r = *rect_item;
