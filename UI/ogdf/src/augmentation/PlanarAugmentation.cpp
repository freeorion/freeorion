...
/********************************************************
 * 
 * implementation of class labelStruct
 * 
 *******************************************************/

void labelStruct::removePendant(node pendant)
{
...
void PlanarAugmentation::doCall(Graph& g, List<edge>& L)
{
	...
	m_pGraph = &g;
		...
		cout << "Graph G is planar         = " << pm.planarityTest(*m_pGraph)<< endl;
	...
	while(!m_labels.empty()){
		...
		// output after each round:
			...
			cout         << "           #pendants = " << m_pendants.size() << endl;
				...
				PlanarModule pm;
				cout << "graph is planar == " << pm.planarityTest(*m_pGraph) << endl;
				cout << "graph is biconnected == " << isBiconnected(*m_pGraph) << endl;
			#endif
			cout << endl << flush;
			
			ListIterator<label> labelIt = m_labels.begin();
			int pos = 1;
		...
		cout << "# planarity tests = " << m_nPlanarityTests << endl;
			...
			cout << "resulting Graph is planar = " << pm.planarityTest(*m_pGraph) << endl;
		#endif
		cout << endl << flush;
	#endif
...
//
//  ----------------------------------------------------
void PlanarAugmentation::reduceChain(node p, label labelOld)
{	
	#ifdef PLANAR_AUGMENTATION_DEBUG
...
bool PlanarAugmentation::planarityCheck(node v1, node v2)
{
	...
	PlanarModule pm;
	bool planar = pm.planarEmbed(*m_pGraph);
...
//
//  ----------------------------------------------------
void PlanarAugmentation::removeAllPendants(label& l)
{
	while (l->size() > 0){
...
//	----------------------------------------------------
//	addPendant(label& l, node pendant)
//
// 		adds a pendant p to a label l
//		re-inserts also l to m_labels
//
//  ----------------------------------------------------
void PlanarAugmentation::addPendant(node p, label& l)
{
	m_belongsTo[p] = l;
	l->addPendant(p);
...
//	----------------------------------------------------
//	joinPendants(label& l)
//
// 		connects all pendants of the label
//
//  ----------------------------------------------------
void PlanarAugmentation::joinPendants(label& l)
{
	#ifdef PLANAR_AUGMENTATION_DEBUG
...
//
//  ----------------------------------------------------
void PlanarAugmentation::connectInsideLabel(label& l)
{
	#ifdef PLANAR_AUGMENTATION_DEBUG
...
//
//  ----------------------------------------------------
ListIterator<label> PlanarAugmentation::insertLabel(label l)
{
	if (m_labels.size() == 0){
		return m_labels.pushFront(l);
	}
	else{
		ListIterator<label> it = m_labels.begin();
...
//	----------------------------------------------------
//	deleteLabel(label& l, bool removePendants)
//
// 		deletes a label
//		and - if desired - removes the pendants belonging to l
//
//  ----------------------------------------------------
void PlanarAugmentation::deleteLabel(label& l, bool removePendants)
{
	ListIterator<label> labelIt = m_isLabel[l->parent()];
...
//	----------------------------------------------------
//	connectLabels(label first, label second);
//
...
//
//  ----------------------------------------------------	
void PlanarAugmentation::connectLabels(label first, label second)
{	
...
//
//  ----------------------------------------------------	
label PlanarAugmentation::newLabel(node cutvertex, node p, paStopCause whyStop)
{
	label l = OGDF_NEW labelStruct(0, cutvertex, whyStop);
	l->addPendant(p);
	m_belongsTo[p] = l;
...
//	----------------------------------------------------
//	findMatching(label& first, label& second)
//
...
//
//  ----------------------------------------------------	
bool PlanarAugmentation::findMatching(label& first, label& second)
{
...
//	----------------------------------------------------
//	connectCondition(label a, label b)
//
// 		checks the connect-condition for label a and b
//
//  ----------------------------------------------------
bool PlanarAugmentation::connectCondition(label a, label b)
{
	bool found = false;
