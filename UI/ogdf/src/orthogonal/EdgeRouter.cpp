...
void EdgeRouter::compute_place(node v, NodeInfo& inf/*, int l_sep, int l_overh*/)
{
	...
	int best_start_index = 0; //in horizontal/vertical edge list (paper: ibest)
...
void EdgeRouter::place(node l_v) 
{
	...
	while (it.valid())
	{
		...
		adjEntry cornerpred = ae->cyclicPred();
			...
			|| ((v == e->source()) && !inedge) ))
	    {
			...
			if (inedge) 
			{
				...
				cornerpred = runadj->twin()->cyclicPred();
			...
			switch (abendType(ae))
			{
				...
				case bend_1left: //rerouted single bend
					...
					if (ipos == 0)
					{
						...
						adjEntry ae2 = saveadj->cyclicPred();//cornerpred;//ae->cyclicPred(); //aussen an cage
				...
				case bend_1right: 
					if (!corn)
					{
						corn = true;
						edge newe2;
						adjEntry ae2 = saveadj->cyclicPred();//cornerpred;//ae->cyclicPred();
						edge savedge = ae2->theEdge();
						if (ae2 == (ae2->theEdge()->adjSource())) 
						{
							newe2 = addLeftBend(ae2->theEdge());
					...
					{
						adjEntry ae2 = saveadj->cyclicSucc();//cornerpred;//ae->cyclicSucc();
						adjEntry ae3 = ae2->faceCycleSucc();
						if (ae2 == (ae2->theEdge()->adjSource()))
							unsplit(ae2->theEdge(), ae3->theEdge());
						else  
			...
			switch (abendType(ae))
			{
				...
				case bend_1right:
					...
					if ((!acorn) && (ipos == infos[l_v].flips(odSouth, odWest) - 1))
					{
						acorn = true;
						edge newe2;
						adjEntry ae2 = saveadj->cyclicPred();  //cornerpred;
						edge le = ae2->theEdge();
						if (ae2 == (ae2->theEdge()->adjSource())) 
						{
							newe2 = addLeftBend(ae2->theEdge());
