...
void ClusterOrthoShaper::call(ClusterPlanRep &PG,
	...
	int startBoundBendsPerEdge,
		bool fourPlanar) throw(AlgorithmFailureException)
	{
	...
	forall_edges(e,PG)
	{
		...
		if (F[adjF[e->adjSource()]] != F[adjF[e->adjTarget()]]) 
		{
			...
			cost[newE] = (m_traditional ? clusterTradBendCost(
												PG.getClusterGraph().clusterDepth(
												PG.clusterOfEdge(e)),
												clusterTreeDepth,
											tradBendCost) : 
											clusterProgBendCost(
			...
			cost[newE] = (m_traditional ? clusterTradBendCost(
												...
												clusterTreeDepth,
											tradBendCost) : 
						  clusterProgBendCost(PG.getClusterGraph().clusterDepth(
											PG.clusterOfEdge(e)),
											clusterTreeDepth,
											progBendCost)
											//progBendCost);
