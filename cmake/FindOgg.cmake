...
# ^^^^^^^^^^^^^^^^
#
# This module defines :prop_tgt:`IMPORTED` target ``Ogg::Ogg``, if Ogg has been
# found.
#
# Result Variables
# ^^^^^^^^^^^^^^^^
...
# ::
#
# OGG_INCLUDE_DIRS    - where to find ogg.h, etc. OGG_LIBRARIES       - List of
# libraries when using ogg. OGG_FOUND           - True if ogg found.
#
# Hints ^^^^^
#
# A user may set ``OGGDIR`` environment to a ogg installation root to tell this
...
# Allow OGG_LIBRARY to be set manually, as the location of the ogg library
if(NOT OGG_LIBRARY)
  foreach(search ${_OGG_SEARCHES})
    find_library(
      OGG_LIBRARY_RELEASE
      NAMES ${OGG_NAMES} ${${search}}
      PATH_SUFFIXES lib
    )
    find_library(
      OGG_LIBRARY_DEBUG
      NAMES ${OGG_NAMES_DEBUG} ${${search}}
      PATH_SUFFIXES lib
    )
  endforeach()
...
# handle the QUIETLY and REQUIRED argument and set OGG_FOUND to TRUE if all
  ...
  if(NOT TARGET Ogg::Ogg)
    ...
    if(NOT OGG_LIBRARY_RELEASE AND NOT OGG_LIBRARY_DEBUG)
      set_property(
        TARGET Ogg::Ogg
        APPEND
        PROPERTY IMPORTED_LOCATION "${OGG_LIBRARY}"
      )
    endif()
  endif()
endif()
