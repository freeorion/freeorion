...
# .rst: UseCompilerCache
# --------
#
# This module provides a function to setup a compiler cache tool (currently
# ``ccache``):
#
# find_compiler_cache([PROGRAM <ccache_name>] [QUIET] [REQUIRED]) -- Add the
# compiler cache tool (default to look for ccache on the path) to your build
# through CMAKE_<LANG>_COMPILER_LAUNCHER variables. Also supports XCode. Uses a
# wrapper for XCode and CCache < 3.3. Sets the COMPILER_CACHE_VERSION variable.
...
# This module provides a function to setup a compiler cache tool for XCode
# (currently ``ccache``):
#
# use_compiler_cache_with_xcode() -- Confiure XCode to use ccache.  This must be
# called after the project() statement so that CMAKE_C_COMPILER is already set.

function(use_compiler_cache_with_xcode)
  get_property(RULE_LAUNCH_COMPILE_VAR GLOBAL PROPERTY RULE_LAUNCH_COMPILE)
  message(
    STATUS
      "Use CCache for ${CMAKE_GENERATOR} called with: ccache program ${RULE_LAUNCH_COMPILE_VAR} ${COMPILER_CACHE_VERSION}"
  )
  # This wrapper only is needed for CCache < 3.3 or XCode
  if(RULE_LAUNCH_COMPILE_VAR AND ("${CMAKE_GENERATOR}" STREQUAL Xcode OR "${COMPILER_CACHE_VERSION}"
                                                                         VERSION_LESS 3.2.0)
  )
    file(
