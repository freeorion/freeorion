...
public:
    ...
    [[nodiscard]] auto ProductionCost(EmpireID empire_id, int location_id, const ScriptingContext& context,
                                      int in_design_id = INVALID_DESIGN_ID) const -> float;

    //! Returns the number of turns required to produce this part
    [[nodiscard]] auto ProductionTime(EmpireID empire_id, int location_id, const ScriptingContext& context,
