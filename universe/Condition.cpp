...
namespace {
    ...
    std::map<std::string, bool> ConditionDescriptionAndTest(const std::vector<Condition::ConditionBase*>& conditions,
                                                            ...
                                                            boost::shared_ptr<const UniverseObject> candidate_object/* = 0*/)
    {
...
std::string ConditionFailedDescription(const std::vector<ConditionBase*>& conditions,
                                       boost::shared_ptr<const UniverseObject> candidate_object/* = 0*/,
                                       boost::shared_ptr<const UniverseObject> source_object/* = 0*/)
{
    if (conditions.empty())
...
struct ConditionBase::MatchHelper {
    ...
    bool operator()(boost::shared_ptr<const UniverseObject> candidate) const
...
bool ConditionBase::Eval(boost::shared_ptr<const UniverseObject> candidate) const {
    if (!candidate)
        return false;
    ObjectSet non_matches, matches;
    non_matches.push_back(candidate);
                  ...
                  SearchDomain search_domain/* = NON_MATCHES*/) const
{
    ...
    boost::shared_ptr<const UniverseObject> no_object;
...
namespace {
                               ...
                               ObjectSet& from_set, ObjectSet& to_set)
    {
        ...
        std::multimap<float, boost::shared_ptr<const UniverseObject>> sort_key_objects;
        for (boost::shared_ptr<const UniverseObject> from : from_set) {
        ...
        if (sorting_method == SORT_MIN) {
            ...
            for (std::multimap<float, boost::shared_ptr<const UniverseObject>>::value_type& entry : sort_key_objects) {
                boost::shared_ptr<const UniverseObject> object_to_transfer = entry.second;
            ...
            // from the from_set into the to_set.
            for (std::multimap<float, boost::shared_ptr<const UniverseObject>>::reverse_iterator sorted_it = sort_key_objects.rbegin();  // would use const_reverse_iterator but this causes a compile error in some compilers
                 sorted_it != sort_key_objects.rend(); ++sorted_it)
            {
                boost::shared_ptr<const UniverseObject> object_to_transfer = sorted_it->second;
                ObjectSet::iterator from_it = std::find(from_set.begin(), from_set.end(), object_to_transfer);
                if (from_it != from_set.end()) {
                    *from_it = from_set.back();
                    from_set.pop_back();
                 ...
                 inv_hist_it != inv_histogram.rend(); ++inv_hist_it)
            {
                ...
                std::pair<std::multimap<float, boost::shared_ptr<const UniverseObject>>::const_iterator,
                          std::multimap<float, boost::shared_ptr<const UniverseObject>>::const_iterator> key_range =
                    sort_key_objects.equal_range(cur_sort_key);

                // loop over range, selecting objects to transfer from from_set to to_set
                for (std::multimap<float, boost::shared_ptr<const UniverseObject>>::const_iterator sorted_it = key_range.first;
                          ...
                          SearchDomain search_domain/* = NON_MATCHES*/) const
{
    ...
    if (search_domain == NON_MATCHES) {
        ...
        for (boost::shared_ptr<const UniverseObject> matched_object : matched_objects) {
...
bool EmpireAffiliation::Match(const ScriptingContext& local_context) const {
    boost::shared_ptr<const UniverseObject> candidate = local_context.condition_local_candidate;
    if (!candidate) {
...
namespace {
    struct HomeworldSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            boost::shared_ptr<const Planet> planet = boost::dynamic_pointer_cast<const Planet>(candidate);
...
bool Monster::Match(const ScriptingContext& local_context) const {
    ...
    if (boost::shared_ptr<const Ship> ship = boost::dynamic_pointer_cast<const Ship>(candidate))
        if (ship->IsMonster())
            return true;
                                             ...
                                             ObjectSet& condition_non_targets) const
{
    ...
    if (m_type) {
        found_type = true;
        //std::vector<boost::shared_ptr<const T>> this_base;
        switch (m_type->Eval()) {
            case OBJ_BUILDING:
...
namespace {
    struct BuildingSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            // is it a building?
            boost::shared_ptr<const ::Building> building = boost::dynamic_pointer_cast<const ::Building>(candidate);
            if (!building)
                return false;
...
namespace {
    struct ContainsSimpleMatch {
            ...
            m_subcondition_matches_ids()
        {
            ...
            // gather the ids
            for (boost::shared_ptr<const UniverseObject> obj : subcondition_matches) {
                if (obj)
...
std::string InSystem::Description(bool negated/* = false*/) const {
    ...
    if (boost::shared_ptr<const System> system = GetSystem(system_id))
                                                 ...
                                                 ObjectSet& condition_non_targets) const
{
    ...
    std::vector<boost::shared_ptr<const UniverseObject>> sys_objs = obj_map.FindObjects(system_object_ids);
...
std::string ObjectID::Description(bool negated/* = false*/) const {
    ...
    if (boost::shared_ptr<const System> system = GetSystem(object_id))
                                                 ...
                                                 ObjectSet& condition_non_targets) const
{
    ...
    boost::shared_ptr<UniverseObject> obj = Objects().ExistingObject(object_id);
    if (obj)
...
namespace {
    struct SpeciesSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            if (boost::shared_ptr<const ::PopCenter> pop = boost::dynamic_pointer_cast<const ::PopCenter>(candidate)) {
            ...
            if (boost::shared_ptr<const ::Building> building = boost::dynamic_pointer_cast<const ::Building>(candidate)) {
                boost::shared_ptr<const ::Planet> planet = GetPlanet(building->PlanetID());
...
bool Species::Match(const ScriptingContext& local_context) const {
    ...
    // is it a ship?
    boost::shared_ptr<const Ship> ship = boost::dynamic_pointer_cast<const Ship>(candidate);
    if (ship) {
        if (m_names.empty()) {
...
namespace {
    struct FocusTypeSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            boost::shared_ptr<const ResourceCenter> res_center = boost::dynamic_pointer_cast<const ResourceCenter>(candidate);
            boost::shared_ptr<const ::Building> building;
            if (!res_center && (building = boost::dynamic_pointer_cast<const ::Building>(candidate))) {
                if (boost::shared_ptr<const Planet> planet = GetPlanet(building->PlanetID()))
...
namespace {
    struct StarTypeSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            boost::shared_ptr<const System> system = GetSystem(candidate->SystemID());
...
namespace {
    struct ProducedByEmpireSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            else if (boost::shared_ptr<const ::Building> building = boost::dynamic_pointer_cast<const ::Building>(candidate))
...
namespace {
    struct WithinDistanceSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            for (boost::shared_ptr<const UniverseObject> obj : m_from_objects) {
...
namespace {
    ...
    bool LanesAngularlyTooClose(boost::shared_ptr<const UniverseObject> sys1,
                                boost::shared_ptr<const UniverseObject> lane1_sys2,
                                boost::shared_ptr<const UniverseObject> lane2_sys2)
    {
    ...
    // acceptable
    bool ObjectTooCloseToLane(boost::shared_ptr<const UniverseObject> lane_end_sys1,
                              boost::shared_ptr<const UniverseObject> lane_end_sys2,
    ...
    bool LanesCross(boost::shared_ptr<const System> lane1_end_sys1,
                    boost::shared_ptr<const System> lane1_end_sys2,
                    boost::shared_ptr<const System> lane2_end_sys1,
                    boost::shared_ptr<const System> lane2_end_sys2)
    {
    ...
    bool LaneCrossesExistingLane(boost::shared_ptr<const System> lane_end_sys1,
                                 boost::shared_ptr<const System> lane_end_sys2)
    {
        ...
        for (boost::shared_ptr<const System> system : objects.FindObjects<System>()) {
            ...
            for (const std::map<int, bool>::value_type& lane : sys_existing_lanes) {
                boost::shared_ptr<const System> lane_end_sys3 = GetSystem(lane.first);
                if (!lane_end_sys3)
                    continue;
    ...
    bool LaneTooCloseToOtherSystem(boost::shared_ptr<const System> lane_end_sys1,
                                   boost::shared_ptr<const System> lane_end_sys2)
    {
        if (!lane_end_sys1 || !lane_end_sys2 || lane_end_sys1 == lane_end_sys2)
            return true;
        ...
        const ObjectMap& objects = Objects();
        std::vector<boost::shared_ptr<const System>> systems = objects.FindObjects<System>();

        // loop over all existing systems, checking if each is too close to a
        // lane between the specified lane endpoints
        for (boost::shared_ptr<const System> system : systems) {
            if (system == lane_end_sys1 || system == lane_end_sys2)
                continue;
    ...
    struct CanAddStarlaneConnectionSimpleMatch {
            ...
            m_destination_systems()
        {
            ...
            // destination objects
            std::set<boost::shared_ptr<const System> > dest_systems;
            for (boost::shared_ptr<const UniverseObject> obj : destination_objects) {
                if (boost::shared_ptr<const System> sys = GetSystem(obj->SystemID()))
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            boost::shared_ptr<const System> candidate_sys = boost::dynamic_pointer_cast<const System>(candidate);
            if (!candidate_sys)
            ...
            for (boost::shared_ptr<const System> destination : m_destination_systems) {
            ...
            for (const std::map<int, bool>::value_type& lane : candidate_sys->StarlanesWormholes()) {
                boost::shared_ptr<const System> candidate_existing_lane_end_sys = GetSystem(lane.first);
                if (!candidate_existing_lane_end_sys)
                    continue;

                // check this existing lane against potential lanes to all destination systems
                for (boost::shared_ptr<const System> dest_sys : m_destination_systems) {
            ...
            for (boost::shared_ptr<const System> dest_sys : m_destination_systems) {
                ...
                for (const std::map<int, bool>::value_type& dest_lane : dest_sys->StarlanesWormholes()) {
                    boost::shared_ptr<const System> dest_lane_end_sys = GetSystem(dest_lane.first);
            ...
            for (std::vector<boost::shared_ptr<const System>>::const_iterator it1 = m_destination_systems.begin();
                 it1 != m_destination_systems.end(); ++it1)
            {
                boost::shared_ptr<const System> dest_sys1 = *it1;

                // don't need to check a lane in both directions, so start at one past it1
                std::vector<boost::shared_ptr<const System>>::const_iterator it2 = it1;
                ++it2;
                for (; it2 != m_destination_systems.end(); ++it2) {
                    boost::shared_ptr<const System> dest_sys2 = *it2;
        ...
        std::vector<boost::shared_ptr<const System>> m_destination_systems;
    };
}
...
bool Stationary::Match(const ScriptingContext& local_context) const {
    ...
    boost::shared_ptr<const Fleet> fleet = boost::dynamic_pointer_cast<const Fleet>(candidate);
    if (!fleet)
        if (boost::shared_ptr<const Ship> ship = boost::dynamic_pointer_cast<const Ship>(candidate))
...
namespace {
    struct ResourceSupplySimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            for (boost::shared_ptr<const UniverseObject> from_object : m_from_objects) {
...
namespace {
    struct OrderedBombardedSimpleMatch {
        ...
        bool operator()(boost::shared_ptr<const UniverseObject> candidate) const {
            ...
            for (boost::shared_ptr<const UniverseObject> obj : m_by_objects) {
