...
bool OrderedDestroyed::operator==(const ConditionBase& rhs) const {
    ...
    const OrderedDestroyed& rhs_ = static_cast<const OrderedDestroyed&>(rhs);
...
namespace {
    struct OrderedDestroyedSimpleMatch {
        OrderedDestroyedSimpleMatch(const ObjectSet& by_objects) :
            m_by_objects(by_objects)
        {}
        ...
        bool operator()(std::shared_ptr<const UniverseObject> candidate) const {
            ...
            // check if any of the by_objects is ordered to destroy the candidate planet
            for (auto& obj : m_by_objects) {
                auto ship = std::dynamic_pointer_cast<const Ship>(obj);
                if (!ship)
                    continue;
...
void OrderedDestroyed::Eval(const ScriptingContext& parent_context,
    ObjectSet& matches, ObjectSet& non_matches,
    SearchDomain search_domain/* = NON_MATCHES*/) const
{
    bool simple_eval_safe = parent_context.condition_root_candidate || RootCandidateInvariant();
    if (simple_eval_safe) {
        ...
        EvalImpl(matches, non_matches, search_domain, OrderedDestroyedSimpleMatch(subcondition_matches));
    }
    else {
        // re-evaluate contained objects for each candidate object
        ConditionBase::Eval(parent_context, matches, non_matches, search_domain);
...
std::string OrderedDestroyed::Description(bool negated/* = false*/) const {
    ...
    return str(FlexibleFormat((!negated)
        ? UserString("DESC_ORDERED_DESTROYED")
        : UserString("DESC_ORDERED_DESTROYED_NOT"))
        % by_str);
}
...
bool OrderedDestroyed::Match(const ScriptingContext& local_context) const {
    ...
    if (!candidate) {
        ErrorLogger() << "OrderedDestroyed::Match passed no candidate object";
        return false;
    }
    ...
    return OrderedDestroyedSimpleMatch(subcondition_matches)(candidate);
}

void OrderedDestroyed::SetTopLevelContent(const std::string& content_name) {
    if (m_by_object_condition)
        m_by_object_condition->SetTopLevelContent(content_name);
}

unsigned int OrderedDestroyed::GetCheckSum() const {
    unsigned int retval{ 0 };

    CheckSums::CheckSumCombine(retval, "Condition::OrderedDestroyed");
    CheckSums::CheckSumCombine(retval, m_by_object_condition);

    TraceLogger() << "GetCheckSum(OrderedDestroyed): retval: " << retval;
    return retval;
}
