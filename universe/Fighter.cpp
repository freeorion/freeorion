...
Fighter::Fighter(EmpireID empire_id, UniverseObjectID launched_from_id, const std::string& species_name,
...
bool Fighter::HostileToEmpire(EmpireID empire_id, const EmpireManager& empires) const {
...
std::shared_ptr<UniverseObject> Fighter::Clone(const Universe& universe, EmpireID empire_id) const {
    auto retval = std::make_shared<Fighter>();
    retval->Copy(*this, universe, empire_id);
    return retval;
}

void Fighter::Copy(const UniverseObject& copied_object, const Universe& universe, EmpireID) {
