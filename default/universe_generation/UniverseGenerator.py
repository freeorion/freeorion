...
def get_empire_name_generator():
    """
    String generator, return random name from string list,
    If string list is empty generate random name.
    """
    names = get_name_list("EMPIRE_NAMES")
    random.shuffle(names)
    while True:
        if names:
            yield names.pop()
        else:
            yield get_random_name(5)

# global list for star system names
star_names = []
...
def get_star_name():
    ...
    #     if name:
    #         return name
    return random.sample(consonants + vowels, 3).upper() + "-" + str(random.randint(1000, 9999))
...
def pick_star_type():
    ...
    #return fo.create_system(star_type, (name or next(star_names), position.x, position.y)
    return star_type
...
def create_universe():
    ...
    if len(group_names) < num_star_groups:
        group_names.extend([get_random_name(6) for _ in range(num_star_groups - len(group_names))])
    ...
    systems = []
    planet_type_summary = {k: 0 for k in planet_types_all}
    for position in system_positions:
        ...
        for orbit in range(0, fo.sys_get_num_orbits(system)):
            ...
            if planet_size in planet_sizes:
                ...
                planet_type_summary[planet_type] += 1
