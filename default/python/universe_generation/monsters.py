...
def generate_monsters(monster_freq, systems):
    ...
    fleet_can_alter_starlanes = {
        ...
        if any(
            [
                universe.getGenericShipDesign(design).ship_hull.hasTag("CAN_ALTER_STARLANES")
    ...
    for fleet_plan in fleet_plans:
        ...
        print("/ can be spawned at", len(fp_location_cache[fleet_plan]), "of", len(systems), "systems")
    ...
    tracked_nest_valid_locations = {nest: len(fo.special_locations(nest, planets)) for nest in nest_name_map}
