...
def get_priority(tech_name):
    ...
    # theories has 0 priority as they don't unlock anything. If it ends up prereq for any other tech those tech
    # will add the theories in.
    theory = 0
    ...
    # TODO stealthy species probably want more stealthy techs, and base one enemies detection
    stealth = 2.2 if rng.random() < 0.1 else 0.2
    ...
    if tech_name in ["DEF_DEFENSE_NET_REGEN_1", "DEF_DEFENSE_NET_REGEN_2"]:
        return defensive
    if tech_name in ["DEF_PLAN_BARRIER_SHLD_1", "DEF_PLAN_BARRIER_SHLD_2", "DEF_PLAN_BARRIER_SHLD_3",
                     "DEF_PLAN_BARRIER_SHLD_4", "DEF_PLAN_BARRIER_SHLD_5"]:
        return defensive
    ...
    # learning
    if tech_name in ["LRN_PHYS_BRAIN", "LRN_TRANSLING_THT", "LRN_PSIONICS", "LRN_GRAVITONICS", "LRN_EVERYTHING",
                     "LRN_MIND_VOID", "LRN_NDIM_SUBSPACE", "LRN_TIME_MECH"]:
        return theory
    if tech_name in ["LRN_ALGO_ELEGANCE", "LRN_ARTIF_MINDS", "LRN_DISTRIB_THOUGHT", "LRN_QUANT_NET",
    ...
    if tech_name in ["LRN_XENOARCH"]:
        if foAI.foAIstate.aggression < fo.aggression.typical:
            return DEFAULT
        # get xenoarcheology when we have ruins, otherwise it is useless
        return 5 if ColonisationAI.gotRuins else USELESS
    ...
    if tech_name in ["LRN_TRANSCEND"]:
        # Transcendence requires so much RP anyway it will be higher and only higher than techs considered useless lol
        return DEFAULT
    ...
    if tech_name in ["SHP_DOMESTIC_MONSTER"]:
        return USELESS  # TODO Tamable space monsters are rare...
    ...
    if tech_name in ["SHP_SOLAR_CONT"]:
        # red star can be turned into has_black_hole by Artificial black hole
        return min(energy, IMMEDIATE if has_red_star or has_black_hole else USELESS)
    ...
    if tech_name in ["SHP_DIAMOND_PLATE", "SHP_XENTRONIUM_PLATE"]:
        # asteroid hull line and neutronium extraction includes better armors
        return USELESS if asteroid == hull or has_neutron_star else armor

    # weapons TODO include these branches into ship-design calculation
    # don't research obsolete weapons if get deathray from ruins
    if tech_name in ["SHP_ROOT_AGGRESSION", "SHP_WEAPON_1_2", "SHP_WEAPON_1_3", "SHP_WEAPON_1_4"]:
        return massdriver if not tech_is_complete("SHP_WEAPON_4_1") else USELESS
    ...
    # detection
    if tech_name in ["SPY_DETECT_1", "SPY_DETECT_2", "SPY_DETECT_3", "SPY_DETECT_4", "SPY_DETECT_5", "SPY_DIST_MOD",
                     "SPY_LIGHTHOUSE"]:
        return detection
        ...
        for element in list(research_queue)[:10]:
            ...
            if not missing_prereqs:
                ...
                print "    %25s allocated %6.2f RP -- missing preReqs: %s -- unlockable items: %s " % \
                      (element.tech, element.allocation, missing_prereqs, unlocked_items)
        print
