...
PHOTO_MAP = {fo.starType.blue: 3, fo.starType.white: 1.5, fo.starType.red: -1, fo.starType.neutron: -1, fo.starType.blackHole: -10, fo.starType.noStar: -10}
...
def survey_universe():
    ...
    print "Unexplored Systems: %s " % [(sysID, (sys and sys.name) or "name unknown") for sysID, sys in zip(un_ex_sys_ids, un_ex_systems)]
    ...
    for spec_name in empire_species:
        this_spec = fo.getSpecies(spec_name)
        print "%s on planets %s; can%s colonize from %d shipyards; has tags %s" % (spec_name, empire_species[spec_name], ["not", ""][spec_name in empire_colonizers],
...
def get_colony_fleets():
    ...
    colony_targeted_planet_ids = get_colony_targeted_planet_ids(universe.planetIDs, AIFleetMissionType.FLEET_MISSION_COLONISATION)
    ...
    if not colony_fleet_ids:
        ...
        print "Colony FleetIDs: " % FleetUtilsAI.get_empire_fleet_ids_by_role(AIFleetMissionType.FLEET_MISSION_COLONISATION)
    ...
    outpost_targeted_planet_ids = get_outpost_targeted_planet_ids(universe.planetIDs, AIFleetMissionType.FLEET_MISSION_OUTPOST)
    outpost_targeted_planet_ids.extend(get_outpost_targeted_planet_ids(universe.planetIDs, AIFleetMissionType.FLEET_MISSION_ORBITAL_OUTPOST))
    ...
    if not outpost_fleet_ids:
        ...
        print "Outpost FleetIDs: %s" % FleetUtilsAI.get_empire_fleet_ids_by_role(AIFleetMissionType.FLEET_MISSION_OUTPOST)
    ...
    outpost_cost = AIDependencies.OUTPOST_POD_COST * (1 + AIDependencies.COLONY_POD_UPKEEP * len(list(AIstate.popCtrIDs)))
    ...
    evaluated_colony_planet_ids = list(unowned_empty_planet_ids.union(AIstate.outpostIDs) - set(colony_targeted_planet_ids))  # places for possible colonyBase
    ...
    if tech_is_complete(AIDependencies.OUTPOSTING_TECH):
        for pid in (set(reserved_outpost_base_targets) - set(outpost_targeted_planet_ids)):
            ...
            if this_score < 100:
                print "Potential outpost base (rejected) for %s to be built at planet id(%d); outpost score %.1f" % (((planet and planet.name) or "unknown"), loc, this_score)
                continue
            best_ship, col_design, build_choices = ProductionAI.getBestShipInfo(EnumsAI.AIPriorityType.PRIORITY_PRODUCTION_ORBITAL_OUTPOST, loc)
            if not best_ship:
    ...
    evaluated_outpost_planet_ids = list(unowned_empty_planet_ids - set(outpost_targeted_planet_ids) - set(colony_targeted_planet_ids) - set(reserved_outpost_base_targets))

    # print "Evaluated Outpost PlanetIDs: " + str(evaluated_outpost_planet_ids)

    evaluated_colony_planets = assign_colonisation_values(evaluated_colony_planet_ids, AIFleetMissionType.FLEET_MISSION_COLONISATION, None, empire)
    ...
    all_colony_opportunities.update(assign_colonisation_values(evaluated_colony_planet_ids, AIFleetMissionType.FLEET_MISSION_COLONISATION, None, empire, [], True))
    ...
    evaluated_outpost_planets = assign_colonisation_values(evaluated_outpost_planet_ids, AIFleetMissionType.FLEET_MISSION_OUTPOST, None, empire)
    colonization_timer.stop()
    ...
    for planet_id, score in sorted_outposts:
        if score > 0.5:
            print "   %5s | %5s | %s | %s " % (score, planet_id, universe.getPlanet(planet_id).name, list(universe.getPlanet(planet_id).specials))
    print
...
def get_outpost_targeted_planet_ids(planet_ids, mission_type):
    ...
    for planetID in planet_ids:
        ...
        for outpost_fleet_mission in outpost_fleet_missions:
            ai_target = universe_object.Planet(planetID)
...
def assign_colonisation_values(planet_ids, mission_type, species, empire, detail=None, return_all=False):  # TODO: clean up suppliable versus annexable
...
def evaluate_planet(planet_id, mission_type, spec_name, empire, detail=None):
    ...
    if (spec_name != planet.speciesName and
    ...
    have_existing_presence = False
    if AIstate.colonizedSystems.get(this_sysid, []) not in [[], [planet_id]]:  # if existing presence is target planet, don't count
        have_existing_presence = True
    ...
    for bld_type in set(planet_build_names).intersection(AIDependencies.building_supply):
        planet_supply += sum([AIDependencies.building_supply.get(bld_type, {}).get(int(psize), 0) for psize in [-1, planet.size]])
    ...
    neighbor_threat_ratio = ((sys_status.get('neighborThreat', 0)) / float(cur_best_mil_ship_rating)) + min(0, fleet_threat_ratio)  # last portion gives credit for inner extra defenses
    myrating = sys_status.get('my_neighbor_rating', 0)
    jump2_threat_ratio = ((sys_status.get('jump2_threat', 0) - myrating) / float(cur_best_mil_ship_rating)) + min(0, neighbor_threat_ratio)  # last portion gives credit for inner extra defenses
    ...
    if system:
        ...
        if tech_is_complete("PRO_SOL_ORB_GEN") or "PRO_SOL_ORB_GEN" in empire_research_list[:5]:
            if system.starType in [fo.starType.blue, fo.starType.white]:
                if not claimed_stars.get(fo.starType.blue, []) + claimed_stars.get(fo.starType.white, []):
                    ...
                    star_bonus += 10 * discount_multiplier * backup_factor  # still has extra value as an alternate location for solar generators
                    detail.append("PRO_SOL_ORB_GEN BW Backup Location %.1f" % (10 * discount_multiplier * backup_factor))
        ...
        if system.starType in [fo.starType.blackHole] and fo.currentTurn() > 100:
            if not already_got_this_one:
                star_bonus += 10 * discount_multiplier * backup_factor   # whether have tech yet or not, assign some base value
        ...
        if tech_is_complete(AIDependencies.PRO_SINGULAR_GEN) or AIDependencies.PRO_SINGULAR_GEN in empire_research_list[:8]:
            if system.starType in [fo.starType.blackHole]:
                if not claimed_stars.get(fo.starType.blackHole, []):
                    ...
                    star_bonus += 100 * discount_multiplier * backup_factor  # still has extra value as an alternate location for generators & for blocking enemies generators
                ...
                star_bonus += 40 * discount_multiplier*backup_factor*rfactor  # can be used for artificial black hole
        ...
        if tech_is_complete("PRO_NEUTRONIUM_EXTRACTION") or "PRO_NEUTRONIUM_EXTRACTION" in empire_research_list[:8]:
            if system.starType in [fo.starType.neutron]:
                if not claimed_stars.get(fo.starType.neutron, []):
                    ...
                    star_bonus += 20 * discount_multiplier*backup_factor  # still has extra value as an alternate location for generators & for bnlocking enemies generators
        ...
        if tech_is_complete("SHP_ENRG_BOUND_MAN") or "SHP_ENRG_BOUND_MAN" in empire_research_list[:6]:
            if system.starType in [fo.starType.blackHole, fo.starType.blue]:
                ...
                elif this_sysid not in (claimed_stars.get(fo.starType.blackHole, []) + claimed_stars.get(fo.starType.blue, [])):
                    colony_star_bonus += 0.5 * init_val * backup_factor  # still has extra value as an alternate location for energy shipyard
    ...
    if (mission_type == AIFleetMissionType.FLEET_MISSION_OUTPOST or
        ...
        for special in planet_specials:
            if "_NEST_" in special:
                nest_val = NEST_VAL_MAP.get(special, 5) * discount_multiplier*backup_factor # get an outpost on the nest quick
                retval += nest_val
        ...
        if system:
            ...
            for pid in system.planetIDs:
                ...
                if other_planet.size == fo.planetSize.gasGiant:
                    gg_list.append(pid)
                if pid != planet_id and other_planet.owner == empire.empireID and AIFocusType.FOCUS_INDUSTRY in list(other_planet.availableFoci)+[other_planet.focus]:
        ...
        if got_asteroids:
            ...
            if tech_is_complete("SHP_ASTEROID_HULLS") or "SHP_ASTEROID_HULLS" in empire_research_list[:11]:
                if species and species.canProduceShips:
                    ...
                    detail.append("Asteroid ShipBuilding from %s %.1f" % (p2.name, discount_multiplier*20*pilot_val))
        if got_gg:
        ...
        detail.append("baseMaxPop size*psm %d * %d * %.2f = %d" % (planet_size, pop_size_mod, pop_tag_mod, max_pop_size))
        ...
        base_pop_ind = 0.2
        ind_mult = 1 * max(ind_tag_mod, 0.5 * (ind_tag_mod+res_tag_mod))  # TODO: repport an actual calc for research value
        ...
        ind_val = project_ind_val(cur_pop, max_pop_size, cur_industry, max_ind_factor, flat_industry, discount_multiplier)
        ...
        for special in [spec for spec in planet_specials if spec in AIDependencies.metabolismBoosts]:
            gbonus = discount_multiplier * base_pop_ind * ind_mult * empire_metabolisms.get(AIDependencies.metabolismBoosts[special], 0)  # due to growth applicability to other planets
            growth_val += gbonus
        ...
        retval += max(ind_val + asteroid_bonus + gas_giant_bonus, research_bonus, growth_val) + fixed_ind + fixed_res + supplyval
...
def assign_colony_fleets_to_colonise():
    universe = fo.getUniverse()
    empire = fo.getEmpire()
    all_outpost_base_fleet_ids = FleetUtilsAI.get_empire_fleet_ids_by_role(AIFleetMissionType.FLEET_MISSION_ORBITAL_OUTPOST)
    ...
    for fid in avail_outpost_base_fleet_ids:
        ...
        target_id = -1
        best_score = -1
        for pid, rating in assign_colonisation_values(targets, AIFleetMissionType.FLEET_MISSION_OUTPOST, None, empire).items():
...
def send_colony_ships(colony_fleet_ids, evaluated_planets, mission_type):
    ...
    potential_targets = [(pid, (score, specName)) for (pid, (score, specName)) in evaluated_planets if score > (0.8 * cost)]
    ...
    while fleet_pool and potential_targets:
        ...
        planet = universe.getPlanet(planet_id)
        sys_id = planet.systemID
        if foAI.foAIstate.systemStatus.setdefault(sys_id, {}).setdefault('monsterThreat', 0) > 2000 or fo.currentTurn() < 20 and foAI.foAIstate.systemStatus[sys_id]['monsterThreat'] > 200:
            print "Skipping colonization of system %s due to Big Monster, threat %d" % (PlanetUtilsAI.sys_name_ids([sys_id]), foAI.foAIstate.systemStatus[sys_id]['monsterThreat'])
            already_targeted.append(planet_id)
            continue
        this_spec = target[1][1]
        found_fleets = []
        try:
            this_fleet_list = FleetUtilsAI.get_fleets_for_mission(nships=1, target_stats={}, min_stats={}, cur_stats={}, species=this_spec, systems_to_check=[sys_id], systems_checked=[],
                                                         fleet_pool_set=fleet_pool, fleet_list=found_fleets, tried_fleets=set(), verbose=False)
        except:
            continue
