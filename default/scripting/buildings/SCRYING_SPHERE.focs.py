...
BuildingType(  # type: ignore[reportUnboundVariable]
    ...
    buildtime=5,
    location=(Planet() & OwnedBy(empire=Source.Owner) & IsBuilding(name=["BLD_SCRYING_SPHERE"])),
    effectsgroups=[
        *SPECIES_LIKES_OR_DISLIKES_BUILDING_STABILITY_EFFECTS,
        ...
        EffectsGroup(
            scope=(
                (Planet() & IsBuilding())
                | Contains(IsBuilding(name=["BLD_SCRYING_SPHERE"]))
                | OwnedBy(empire=Source.Planet.Owner)  # would be redundant to re-assign ownership to own planets)
                | (System & Contains(IsBuilding(name=["BLD_SCRYING_SPHERE"])))
            ),
            activation=ContainedBy(Planet() & Unowned),
            effects=[
                SetVisibility(empire=Source.Planet.Owner, visibility=MaxOf("Visibility", Partial, Value)),
            ],
        ),
    ],
