...
BuildingType(  # type: ignore[reportUnboundVariable]
    ...
    location=(
        ...
        & Number(low=0, high=0, condition=IsBuilding(name=["BLD_IMPERIAL_PALACE"]))
        & OwnedBy(empire=Source.Owner)
    ...
    enqueuelocation=(  # must own production location planet
        ...
        & TargetPopulation(low=1)
        # can't enqueue if already own a self-built palace        Number low = 0 high = 0 condition = And[Building name = "BLD_IMPERIAL_PALACE"            OwnedBy empire = Source.Owner            ProducedByEmpire empire = Source.Owner] // can't enqueue if already have an enqueued palace anywhere        Number low = 0 high = 0 condition = And [            Planet            Enqueued                type = Building                name = "BLD_IMPERIAL_PALACE"                empire = Source.Owner                low = 1        ]    ],
    ),
    effectsgroups=[
        *SPECIES_LIKES_OR_DISLIKES_BUILDING_STABILITY_EFFECTS,
        ...
        EffectsGroup(
            ...
            effects=SetSpeciesTargetOpinion(
                ...
                * (
                    StatisticIf(float, IsTarget & SpeciesLikes(name=Source.Planet.Species))
                    - StatisticIf(float, IsTarget & SpeciesDislikes(name=Source.Planet.Species))
                ),
            ),
        ),
