...
CAN_ADD_STARLANE_TO_SOURCE = (
    System
    & ~Contains(scope=IsSource)
    & ~HasStarlane(condition=Object(id=Source.SystemID))
    ...
    & ~StarlaneToWouldCrossExistingStarlane(condition=Object(id=Source.SystemID))
    & ~StarlaneToWouldBeAngularlyCloseToExistingStarlane(condition=Object(id=Source.SystemID), maxdotprod=0.87)
    & ~StarlaneToWouldBeCloseToObject(
        distance=20.0,
        condition=Object(id=Source.SystemID),
        closeto=(
            ...
            & ~Contains(scope=IsSource)
            & ~ContainedBy(Contains(scope=IsSource))
            & ~IsRootCandidate
            & ~Contains(scope=IsRootCandidate)
            & ~ContainedBy(Contains(scope=IsRootCandidate))
        ),
    )
)
