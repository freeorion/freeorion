...
def TARGET_TERRAFORMING(arg1, target, arg3, arg4, arg5, arg6, arg7, arg8):
    """
    ...
    BuildingType(  # type: ignore[reportUnboundVariable]
        ...
        * (
            ...
            * StatisticIf(
                float,
                condition=IsSource
                & EmpireHasAdoptedPolicy(empire=Source.Owner, name="PLC_TERRAFORMING")
                * (
                    Target.HabitableSize
                    * max(1, PlanetTypeDifference(from_=Target.OriginalType, to=target))
                    ...
                    * (
                        ...
                        * StatisticIf(
                            float,
                            condition=IsSource
                            & EmpireHasAdoptedPolicy(name="PLC_ENVIRONMENTALISM", empire=Source.Owner),
                        )
                    )
                ),
            )
        ...
        buildtime=12
        - 6
        * StatisticIf(int, condition=IsSource & EmpireHasAdoptedPolicy(empire=Source.Owner, name="PLC_TERRAFORMING")),
        location=(
            Planet()
        ...
        enqueuelocation=(
            ...
            & ~(
                ...
                | (
                    ...
                    | Enqueued(type=BuildBuilding, name=f"BLD_TERRAFORM_{arg5}")
                )
            )
            & ~(
                Planet(type=arg3)
                & (
                    Contains(IsBuilding(name=[f"BLD_TERRAFORM_{arg7}"]))
...
BuildingType(  # type: ignore[reportUnboundVariable]
    ...
    location=(
        ...
        & ~Contains(IsBuilding(name=["BLD_GAIA_TRANS"]))
        & IsBuilding(name=["BLD_GAIA_TRANS"])
        & HasSpecial(name="GAIA_SPECIAL")
