...
BOOST_PYTHON_MODULE(_effects_new) {
    ...
    py::def("CreatePlanet", boost::python::raw_function(insert_create_planet));

    py::def("EffectsGroup", py::raw_function(insert_effects_group_));
    py::def("Item", py::raw_function(insert_item_));

    py::scope().attr("Destroy") = effect_wrapper(std::make_shared<Effect::Destroy>());
    py::scope().attr("NoOpEffect") = effect_wrapper(std::make_shared<Effect::NoOp>());

    py::def("GenerateSitRepMessage", py::raw_function(insert_generate_sit_rep_message_));
    py::def("Conditional", py::raw_function(insert_conditional_));

    py::def("SetEmpireMeter", py::raw_function(set_empire_meter));
    py::def("Victory", py::raw_function(victory));
    py::def("AddSpecial", py::raw_function(add_special));
    py::def("SetSpecialCapacity", py::raw_function(add_special));
    py::def("RemoveSpecial", py::raw_function(remove_special));
    py::def("CreateShip", py::raw_function(create_ship));
    py::def("CreateBuilding", py::raw_function(create_building));
            ...
            {"SetSize",               MeterType::METER_SIZE}})
    {
        ...
        py::def(meter.first, boost::python::raw_function([m](const boost::python::tuple& args, const boost::python::dict& kw) { return insert_set_meter_(m, args, kw); }));
    }
    ...
    {
        const auto m = meter.second;
        py::def(meter.first, boost::python::raw_function([m](const boost::python::tuple& args, const boost::python::dict& kw) { return insert_ship_part_set_meter_(m, args, kw); }));
    }

    py::def("SetEmpireStockpile", py::raw_function(insert_set_empire_stockpile));
    py::def("SetEmpireCapital", py::raw_function(insert_set_empire_capital));
    py::def("SetOwner", py::raw_function(insert_set_owner_));
    py::def("SetStarType", py::raw_function(insert_set_star_type_));
    py::def("MoveTo", py::raw_function(insert_move_to_));
    py::def("MoveTowards", py::raw_function(insert_move_towards_));
    py::def("SetPlanetSize", py::raw_function(insert_set_planet_size_));
    py::def("SetPlanetType", py::raw_function(insert_set_planet_type_));
    py::def("SetVisibility", py::raw_function(insert_set_visibility_));
    py::def("AddStarlanes", py::raw_function(insert_add_starlanes_));
    py::def("RemoveStarlanes", py::raw_function(insert_remove_starlanes_));
            ...
            {"GiveEmpirePolicy", UnlockableItemType::UIT_POLICY}})
    {
        ...
        py::def(uit.first, py::raw_function([u](const boost::python::tuple& args, const boost::python::dict& kw) { return insert_give_empire_item_(u, args, kw); }));
    }
